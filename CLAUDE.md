...
- **本番へ出る扉は `v*` タグだけである。** main への push はどこにも配らない。
  ...
  **版数ゲート → CI 全部 → 重量級コーパス → 本番展開 → マニュアル（PDF） → 証拠と GitHub Release**
  の順に回す（実測で 40 分強。マニュアルの段を足してからの実測は無い）。
  **本番展開までの 1 つでも落ちれば本番へは出ない。** マニュアルだけは本番展開のあとで作り、
  **落ちても配信を止めない**——落ちたことは証拠に書く（`tools/release-evidence.mjs` の
  `MAY_FAIL` はこの 1 つだけ。マニュアルの設計書 §5、利用者の裁定）。
