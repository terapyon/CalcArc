...
- **`errors-000.json` は全領域の寄せ集め 28 件**である。除算・対数・平方根・
  逆三角・極・階乗・組合せ・値域が 1 枚に同居している
  （**2026-08-30 の時点では 30 件で、括弧も同居していた**——**括弧の 2 件は
  0.9.3 の C-6 で退役した**。**この節の論旨は件数に依らない**ので、
  「足りなかったのは件数ではなく場所と体系性である」は動かない）
