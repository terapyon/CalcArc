...
独立: 不可能（式は spec §3 の契約で同じ／評価はどちらも厳密な整数／**上限の当て方も
同じ**——spec §3.6 が「左から順に、その都度」と定めており、こちらもそう書いている
（`_product` の docstring）。**別解が無い。**
**`data_scale_ref` は最後に 1 度だけ当てるが、判定が割れる入力が無いので同じく `不可能` である**）
"""
