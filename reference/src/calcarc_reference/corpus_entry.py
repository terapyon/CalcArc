...
独立: 不可能（**打鍵の途中の表示に数学的な定義は無い。**規則を持つのは
`engine_table.rs` と `state.rs` だけで、ここはそこから導く。独立に計算していない
——だから一致は独立の証拠にならない）
"""
...
def sign_toggle_cases() -> list[dict]:
    ...
    return [
        ...
        _case(
            ["1", "2", "neg", "add", "3", "eq"],
            "-9",
            "+/- の答え(-12)は手元の値になり、次の演算(+)の左の被演算子になる。"
            "+/- の直後に新しい桁を打つことはできない——手元の値があるあいだ数字キーは"
            "拒まれる(0.9.2 設計書 F5、engine_table.rs:1200 の "
