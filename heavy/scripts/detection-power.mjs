...
export const MUTATIONS = [
  ...
  },
  {
    ...
    // 検出数は 910**(走行 `34463437618` の `heavy-report.md`。v0.8.0 は 622、
    // Release 走行 `33942305098`)。**910 = 前と同じ範囲 868 件の中で 622 +
    // `compound_periods_for` 288**——この内訳は CI の成果物からは分けられず、
    // 重量級の担当が手元で 1 本だけ回して分けた(2026-09-10)。**前と同じ
    // 範囲では v0.8.0 と一致したので弱くなっていない。増えた 288 は `step()` に
  ...
  },
  {
    ...
    // **2026-09-10 に `step()` への集約に合わせて当て直した。当て直し後の
    // 検出数は 1 で、v0.8.0 と同じ**(走行 `34463437618` の `heavy-report.md`。
    // v0.8.0 は Release 走行 `33942305098`)。`minRate` は置かないまま。`from` は
    ...
    // を返していた(税なし 79 件・税あり 72 件。税なしは期数について厳密に
