...
**Pull Request は歓迎する。** ただし取り込まれたコード・ドキュメントの権利は
**Manabu TERADA（GitHub: `terapyon`）に帰属する。**

**同意のしかたは 1 つだけ**——[CONTRIBUTORS.md](CONTRIBUTORS.md) に自分の名前と
GitHub ID を書き足す。**その追記を含む Pull Request をもって、同意したものとみなす。**
初めての PR にその 1 行を含めてくれればよく、2 回目以降は要らない。

権利を集約するのは、**将来ライセンスを変える必要が出たときに、全員に連絡を取り直さ
なくて済むようにする**ためである。プロジェクトは Apache License 2.0 で公開しており、
**貢献したコードは同じライセンスで誰でも使える状態のまま**である。
...
- **参照実装を Rust の移植にしない。** 同じアルゴリズムを両方に書くと、
  ...
  **宣言の書き方は下の「参照実装を足すとき」に従う。**
- **規律を書いたら、同じコミットで番人を置く。** ここに並んでいる規律は、
  どれも「書いてある」だけでは守られない。**番人とは、破ったときに赤くなる
  もの**である——テスト、lint の規則、CI の 1 段、生成器の assert。
  **番人を置けないなら、置けない理由と、代わりに何が守るのかを書く。**
  たとえば「参照実装を移植にしない」は**機械では検出できない**
  ——同じ手順で書けば、照合は一致して緑になる。だからそこは
  **宣言の痕跡**（下記）で守り、それがレビューの足場になる。
...
`reference/` に計算を足すときは、**Rust とどう違うのかを書く。**

**なぜ**: 「移植にしない」は**機械では検出できない**。同じ手順で書いても
照合は一致して緑になるので、**規律を守った証拠は宣言しか残らない**。
宣言が無いと、読む人は「必然的に同じ」と「移植したから同じ」を区別できず、
**規律はレビューする人が代わった瞬間に消える**。

### 書く場所は 2 つ

1. **設計書に節を 1 つ**（`docs/superpowers/specs/…`）。公開する関数を並べ、
   それぞれ下の 3 つのどれかを書く。
2. **公開関数の docstring に 1 行**。設計書は読まれないことがあるが、
   **コードは必ず読まれる。**

### 書き方は 3 つだけ

```
独立: 別手順（Rust は標準の指数書式に丸めさせて指数を読む。
      こちらは Decimal を有効数字 10 桁で丸めて adjusted() で読む）
独立: 不可能（floor(b·r) に別解が無い。一致は独立の証拠にならない）
独立: 未確認（Rust 側を読んでいない。読むまで一致を根拠にしない）
```

上の 1 つ目は**現物である**——`crates/calcarc-core/src/numeric/format.rs` の
`format_real` と `reference/src/calcarc_reference/eng_ref.py` の
`format_real_eng` は、丸めと桁の数え方を**別の道具に任せている**。
2 つ目も現物で、`loan_ref.monthly_interest` と Rust の
`monthly_interest_floor` は**同じ手順**である（そう書いてあれば、
読む人はその一致を証拠として数えない）。

**「別手順」と書く前に、Rust 側を実際に読むこと。** 確かめずに書けば、
それは**偽の理由**であり、次に読む人を確実に誤らせる——**嘘の理由は、
無い理由より悪い。**

**「不可能」を恥じない。** 一意な式に別解は無い。**そう書いてあれば、
読む人は「この一致は何も証明しない」と正しく読める。**
