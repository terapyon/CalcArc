...
///
/// **再実測(2026-09-17、0.9.3 の D-2 のあと)**: **421,383 回**。同じ理由で、
/// **開いていない `)` の枝が落ちた**(利用者の裁定 2026-09-17)。
///
/// **差は測って確かめた**。この先端で `refuses` の `)` の分岐だけを外すと
/// **426,556 回**になる——つまり**減った 5,173 回はすべて、開いていない `)` を
/// 含む列である**。**426,191 ではなく 426,556 なのは D-1 のぶんである**
/// (閉じた組を DEL が開き直すので、網に無かった状態が 365 回ぶん増えた。
/// あちらは下限を上回る向きなので緑のままだった)。
///
/// **落ちた列で綴りの照合を失っていない。** あれは**押した瞬間にエラーになる列**で
/// あり、`check` が比べるのは**入力中の綴り**である——エラーのあとに `buffer` は
/// 開かない。**失ったのは「エラーに落ちたあとも綴りが一致する」という照合**で、
/// それは `engine_table.rs` の行が名前付きで持っている。
///
...
fn every_sequence_up_to_five_keys_spells_the_entry_as_the_engine_shows_it() {
    ...
    assert!(
        compared >= 421_383,
        "網羅で比べたのは {compared} 回(2026-09-17・D-2 のあとの実測 421,383 回)。\
