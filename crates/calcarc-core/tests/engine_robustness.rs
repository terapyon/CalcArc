...
/// (15/14)^6 = 1.50 と一致する。**長さ 6 を維持した**——毎タスク回す検証段
/// (CONTRIBUTING の段付け)に収まる範囲であり、`Exp` こそ状態機械の形を
/// 変えるキーで、網から外すと一番見たいものが見えなくなるため。次に等価類を
/// 足す人へ: 16 個なら約 11 秒、17 個なら約 16 秒になる。そこが分かれ目で、
/// 越えるなら長さを 5 に落として `Exp` を含む列の重点列挙を別に持つこと。
