...
/// 300 列):
///
/// **再計量(2026-09-17、0.9.3 の D-2)**: **開いていない `)` が押せなくなった**
/// (利用者の裁定 2026-09-17)。**この生成器にとって最大のエラー源が消えた**
/// ので、**復帰回数が 1490 → 523 に落ちた**。**滞在量は 270 のまま動かない**
/// ——深さを作るのは「演算子のあとの `(`」であって `)` ではない。
/// **表は生成器ごと撮り直した**(この種で 300 列):
///
/// | 生成器 | 最大深さ | 深さ 5 以上の打鍵数 | 復帰回数 |
/// |---|---|---|---|
/// | 現状(演算子のあとの `(` が 3) | 8 | 270 | 523 |
/// | 演算子のあとの `(` を 3 → 2 | 8 | 99 | 487 |
/// | `)` を 2 → 6 | 7 | 63 | 477 |
/// | 重みを全部 1 | 8 | 56 | 660 |
///
...
///
/// **★ 復帰回数の役割は 0.9.3 で弱くなった。正直に書く。**
/// 0.9.2 の註は「生成器が悪化するほど復帰は単調に増える」と言っていて、
/// 実際 3 通りとも 1490 より上(1706・3391・2008)だった。**D-2 でその話は
/// 崩れた**——**開いていない `)` がエラーを作らなくなった**ので、`)` を厚く
/// しても復帰は増えず(477)、`(` を薄くしても増えない(487)。**いま窓
/// `450..=650` が捕まえるのは「重みを全部 1」(660)だけ**で、**残りの 2 通りは
/// 窓の内側に入る**(あれらは滞在量が捕まえる。99・63 対 閾値 150)。
/// **下限 450 の仕事は「復帰の経路がまだ踏まれている」ことの確認**であり、
/// **崩れの検出は滞在量が持つ。**
///
/// **★ ただし、上限には新しい仕事が生まれている**（calcarc-1e の指摘、2026-09-17。
/// **私も同じ手順で測って 1486 回を確かめた**）。**`refuses` の `)` の分岐を外すと
/// 復帰は 1486 回**になり、**窓の外へ出て赤くなる**——つまりこの窓は、いまや
/// **「`)` が押せる世界に戻ったこと」を乱歩の側から捕まえる**。
/// **重みの崩れは滞在量、D-2 の退行はこの窓**、と仕事が分かれた。
/// **窓を「もう何も見ていない」と読んで外さないこと。**
///
...
fn the_weighted_search_still_reaches_deep_states() {
    ...
    assert!(
        (450..=650).contains(&recoveries),
        "エラーからの復帰が {recoveries} 回（0.9.3 D-2 後の実測 523。崩した 3 通りは\
         487・477・660 で、窓の外に出るのは 660 の 1 通りだけである——**崩れの検出は\
         上の滞在量が持つ**。この窓は「復帰の経路がまだ踏まれている」ことを言う）。\
