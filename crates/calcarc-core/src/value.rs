...
impl Value {
    ...
    ///
    /// **残っている差は 8 件で、すべて 1 ULP** である（実部・虚部とも ULP 差 1、
    /// 相対 2.2e-16 / 1.1e-16）。**Smith 法は丸めを 2 回通す**ので、
    /// **正しく丸めた商とのビット一致は保証できない。**
    ///
    /// **★ その 8 件は「非正規化域が丸ごと不正確」ではない**（calcarc-88 の
    /// 切り分け、2026-09-18。**私も 8 件を全部印字して確かめた**）:
    /// **`e-320` の `double` 族（`z/(z÷2)`）の、実部優勢と虚部優勢だけ**である
    /// ——**`self`・`half`・`real_over_sym` は 1 件も含まれない**。
    /// **`z÷2` という「非正規化域で入力を作る演算」が絡んだときだけ**であり、
    /// **直した当のもの（下端の相対 2.4e-4）とは別の話**である。
    /// **混ぜて読むと「下端は信用できない」になる。**
    ///
    /// # 呼び出し元まで届いているか（**測った**）
    ///
    ...
    /// いまは `j`（番人は `scientific/mod.rs` の
    /// `tangent_of_a_large_imaginary_part_is_not_a_false_overflow`）。
    ///
    /// **★ 単位換算・為替・式の評価は、この壊れ方を持っていない。**
    /// あれらが割るのは **`Rational`**（`expr/rational.rs` の `checked_div`）で、
    /// **f64 を通らない厳密計算**である——**`Value` の除算とは別の関数**。
    /// **「除算の呼び出し元 5 か所」と数えると混ざる**（2026-09-18 に一度混ざった）。
    ///
    /// # 測っていないもの
    ///
    /// **「塞いだ」と読まないこと**——**①をそう読んだのが F7 の出発点だった**。
    ///
    /// - **`checked_mul` / `checked_add` / `checked_sub`**: **論証は在るが実測は無い**
    ///   （calcarc-88: 各部分積は `|a||b|` で抑えられるので、真の積が有限なら
    ///   中間は溢れない——**本人が「論証であって実測ではない」と申告している**）。
    ///   **0.9.4 の範囲外**（監視役の裁定）。**測るなら乗算にも同じ表が要る。**
    /// - **`a/b/c` のように途中の商が端に着地する連鎖**
    /// - **`recip`・極形式**（`tan` は上のとおり測った）
    /// - **表示層**、**wasm 境界の向こう**（**88 が wasm32 で件数を測っている**）
