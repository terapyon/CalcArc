...
mod tests {
    ...
    #[test]
    fn the_binary_base_is_ten_twenty_four_not_a_thousand() {
        // 1010 GiB ちょうど(1010 × 1024^3)。
        //
        // **整数部が 1000 以上 1024 未満に収まる 2 進の値でしか、基数の
        // 取り違えは見えない。** `base` が効くのは「丸めた後の整数部が
        // 基数に達したら 1 つ上の単位で丸め直す」1 か所だけで、上の
        // `format_binary(1_099_460_000_000)` は整数部が 1023.95 → 丸めて
        // 1024 になるため、基数が 1000 でも 1024 でも同じく再選択する
        // ——この帯を通る値でなければ、`units[0].1` を 1000 に取り違えても
        // 表示は 1 文字も変わらない。ここは GiB のまま留まることを主張する
        // (基数を 1000 にすると TiB で丸め直され、実測で `1.0 TiB` に化ける)。
        assert_eq!(
            format_binary(1_084_479_242_240).as_deref(),
            Some("1010.0 GiB")
        );
    }
