//! 複素除算の境界専用 golden（F7、0.9.4）を **製品が走る wasm32** で確かめる。
//!
...
//! ——**印字が切れても崩れない主張**である。
//!
//! **本文は `crates/calcarc-core/tests/complex_div_boundary.rs` と同一**
//! （`EXPECTED_INEXACT` を含む。**片方だけ直すと、native と wasm32 で違う主張になる**）。
//!
