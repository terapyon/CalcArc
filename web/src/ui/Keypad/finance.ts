 ...
 *
 * - **語**: 冠が左 3 つは `返済 / 借入 / 返済`、右 3 つは `複利 / 必要 / 必要`。
 *   **語だけで群は読める**
 * - **色**: 右 3 つが `accent`。`finance.ts` に `accent` は 0 件だった(実測)
 *   ので、**新しい意味を持ち込まずに使える**
 *
 * **★ この関係が逆になってはいけない**——**「色でしか群が分からない」状態を
 * 作らない。** 色が見えない人には語が残る。**註は腐るので、番人の形にして
 * ある**: `finance.test.ts` の「色の群と語の群が同じ」が、**`accent` を持つ
 * キーの集合と、冠が `複利`/`必要` であるキーの集合の一致**を assert する
 * ——**片方だけ動かすと赤くなる。**
 *
...
const MODES: KeypadSection<FinanceKeyToken> = {
  ...
  keys: [
    ...
    // コアも 1 本の関数である(設計書 §6)。
    // **ここから右 3 つが `accent`。** 上の註のとおり、**色は語の補強**である。
    {
      token: "mode:compound",
      label: "複利\n残高",
      ariaLabel: "複利で増やす",
