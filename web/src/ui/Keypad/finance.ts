 ...
 * なので数字の近くに置く(設計書 §2)。**`=` はあるが、答を出すキーではない**
 * ——答は必要な項目が埋まった時点で出る(押さないと出ないキーがあると、
 * 「押していないから」なのか「入力が足りないから」なのかを見分けられない)。
 * `=` は**打っている項目の式を評価して、その値に置き換える**だけである
 * (`FinancePanel.tsx` の `eq`。式が壊れていれば押せない)。
 *
 * **【訂正 2026-09-11】** 以前ここは「`=` は無い」と書いていた。書いたのは
 * 2026-08-14 で、式入力を入れた翌 2026-08-15 に右下へ `=` が入った
 * ——この註だけが追いつかなかった。
 */
