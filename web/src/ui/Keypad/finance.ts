...
const COMPOUND_FIELDS: KeypadSection<FinanceKeyToken> = {
  ...
  keys: [
    ...
    },
    {
      ...
      //
      // **★ 入力済みの chip の見出しは「周期」のまま**である
      // (`FinancePanel.tsx` の `FIELD_LABELS.periods`)。**キーと chip が別の語に
      // なっている**——押した先で何が変わったのかを目で追いにくい。
      // **chip の見出しは裁定の対象に入っていなかった**ので、ここでは変えず、
      // 設計書の未決に立てた。**読み上げ名は正確なので変えない。**
      label: "方式",
