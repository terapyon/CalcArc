...
/**
 * 関数列の 2 段目。**横に 8 列へ広げる道が塞がっている**ので縦に増やした
 * (S-2 設計書 §7.2)。キー幅は 390px で 45.43px のまま。
 *
 * **塞いでいるのは 390px ではなく 360px である。** `Keypad.module.css` の
 * `column-gap` が余りを列間から吸うので、**390px で 8 列にしてもキーは
 * 44.00px ちょうどで、44px を割らない**(列間が 8px から 2px へ潰れる)。
 * 割れる代わりに、**360px では列間を 0 にしても 8 × 44px = 352px が内幅
 * 336px に入らず、区画が親より 16px 広くなる**。
 * **設計書 §7.1 の「390px で 8 列は 38.75px」は `column-gap` の規則
 * (0.2.1、`5ce18be`)が入る前の数字で、いまは再現しない**——§7.1 には
 * その追記がある。
 *
 * **この 2 段化を守っている検査は 1 本だけである**——
 * `web/tests/e2e/viewport-budget.spec.ts` の
 * `no key row is wider than the board that holds it, at 360px`。
 * **44px を測る 2 本では止まらない**(8 列でもキーは 44px ちょうど)し、
 * **溢れを測る 1 本でも止まらない**(区画が 16px はみ出しても、左右の余白の
 * ぶん文書の幅は 4px しか増えず、8px の許容に収まる)。**実際に 8 列 ×
 * 8 キーへ変えて測った**(2026-09-04)。
 *
