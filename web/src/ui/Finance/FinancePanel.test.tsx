...
describe("FinancePanel（電卓）", () => {
  ...
  //
  // **★ ここで見るのは「語」であって文ではない。** 問いは**方式の取り違え**
  // だけなので、**その方式にしか現れない語**を 1 つずつ見る: ローンに
  // `返済額`、複利に `切り捨て`。**複利の文そのもの**(承認済みの綴り)は
  // `src/ui/Keypad/finance.test.ts` の表が 1 字ずつ持つ。以前は「文案が
  // 未決なので綴りを検査に写さない」と書いていたが、2026-09-10 に確定した。
  //
