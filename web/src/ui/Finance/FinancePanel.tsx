 ...
 *
 * **止めているのは小数点だけである。** この数が届くのは `pushDot` の経路
 * (下の `press` の `"dot"`)だけで、**数字は止まらない**——数字は
 * `web/src/finance/entry.ts` の `pushDigit` を通り、そちらの上限は
 * `MAX_YEN_DIGITS = 20` である。
 *
 * **註は「それ以上打てても SyntaxError になるだけなので、入口で止める」と
 * 書いてあった。嘘だった**(2026-09-17 に訂正、0.9.3 設計書 §8.2)。
 * **入口で止まるのは小数点であって、年利の文字数ではない。**
 * 0.9.2 でマニュアル側の同じ誤り(「年利は 8 文字まで」)は直したが、
 * **番人の無い場所——コードの註——に残っていた。**
 */
...
/**
 * 常設の説明。**方式ごとに 1 文字列**(設計書 §4.2.1「常設 2 行だけ。
 * 開閉式は作らない」)。
 *
 * **★ 複利の 2 行は確定した**(利用者の裁定、2026-09-10。設計書 §8 の
 * #1・#3・#6 は決着)。**承認済みの綴りとして、`Keypad/finance.test.ts` の
 * 表が 1 字ずつ守る。** ローンの 1 文は今回の裁定の対象ではない(前からの
 * 文)ので、その表には入れていない。
 *
 * 以前ここには「**文案はまだ動くので、型にも検査にも染み出させない**」と
 * 書いてあった。**未決のあいだは正しい理由だったが、確定した今は無い**
 * ——文が決まったのに文を守らないと、1 字変えても誰も言わない
 * (項目キーの `周期他` がまさにそれで 1 日入っていた)。
 *
...
export function FinancePanel() {
  ...
  return (
    <section className={styles.panel} aria-label="金融計算">
      ...
      {/* **方式で中身が変わる。** ローンと複利では食い違う点が別だから
          である(設計書 §4.2.1)。**要素は 1 つのまま**——`<p>` を 2 つに
          割ると、行数の番人(`finance-layout.spec.ts` の
          `section[aria-label="金融計算"] > p`)が 2 件を数えて落ちる。 */}
      <p className={styles.disclaimer}>
        {isCompoundFamily(mode) ? DISCLAIMER.compound : DISCLAIMER.loan}
      </p>
    </section>
  );
}
