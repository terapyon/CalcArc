 ...
 * 入力中」に入るため)。
 *
 * **これは「4 形のうちの負」を仮数だけの話に限らない**——ただし
 * **仮数が正で指数だけが負の形に限る**(仮数が負で指数もある形は下で
 * 別に扱う)。`crates/calcarc-core/src/numeric/format.rs` の
 * `EXP_LOW_EXPONENT = -9` により、絶対値が 1e-9 未満の答は指数が負の
 * まま普通に出る(大きい数で割るだけで届く)——一覧の見た目は成功した
 * 行と同じ `<button>` なので、ここを写せないままにすると押しても静かに
 * 何も起きない行ができる(Fix round 1 finding 1)。この形はいまも
 * 正しく呼び戻せる。
 *
 * **仮数が負で指数もある形は、0.9.2 以降は写せない。** 仮数の符号は
 * `exp` を送る前でなければならない(上)が、`neg` を送った時点で値は
 * 確定する。0.9.2 より前はその後の `exp` が確定した値を黙って捨てて
 * いた——`-1.5e12` は `1 dot 5 neg exp 1 2` という列を送っていたが、
 * 確定した `-1.5` を捨てて `1e12` が入力欄に残った。0.9.2 の F5
 * (設計書 §9-8)は確定した値のあとの `exp` を拒否に変えたので、いまは
 * 同じ列が `-1.5` を残して止まる。**症状が変わっただけで、写せないと
 * いう欠陥自体は 0.9.2 より前からある。**
 *
 * | 答 | 送っていた列 | 0.9.2 より前 | いま |
 * |---|---|---|---|
 * | `-1.5e12` | `1 dot 5 neg exp 1 2` | `1e12` | `-1.5` |
 * | `-1.5e-12` | `1 dot 5 neg exp 1 2 neg` | `1e-12` | `1.5` |
 *
 * **それでも写せない形は `null` を返す**——虚数(`j`)・極形式(`∠`)・
 * 60 進(`°′″`)、そして上の「仮数が負で指数もある形」のように、そもそも
 * 数字キーの列で表せない(または表しても確定値と食い違う)綴りがそれに
...
function mapAnswerToKeys(
  ...
  // **仮数が負で指数もある形は写せない**(上のドキュメント参照)。`neg` は
  // `exp` の前で送らなければならないが、送った時点で値が確定し、続く
  // `exp` は 0.9.2 設計書 §9-8 で拒否される(拒否される前は黙って値を
  // 捨てていた——欠陥自体は 0.9.2 より前からあり、F5 は症状を変えた
  // だけ)。「写せない形は `null` を返す」規則をここに適用する。
  if (mantissaNegative && exponent !== null) return null;
