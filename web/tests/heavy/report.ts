...
function renderCaveats(entries: ShardSummary[]): string[] {
  ...
  return [
    ...
    "  `dispatch` と wasm の関数だけで、押せる場所に見えるかは既存の",
    "  E2E(Layer 5)の担当である。",
    "",
    "  **これは「まだ試していない」ではなく、この経路からは見えない、が正確である。**",
    "  `web/vite.heavy.config.ts` は React を含まず、入口も `heavy-harness.html`",
    "  の 1 つだけなので、**この走行のブラウザにアプリの画面は存在しない**。",
    "  したがって「盤面のどのボタンからその関数に届くか」——Shift 層の奥にある",
    "  キー、押せない位置にあるボタン、表示に配線し忘れた欄——は、",
    "  **この層が緑でも何も分からない**。",
    "",
