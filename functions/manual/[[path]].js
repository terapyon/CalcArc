 ...
 *
 * **代償**（**仕様からの読み。測っていない**——理由は下）: **PC のブラウザでは
 * PDF がその場で表示されず、保存（ダウンロード）になる**はずである。
 * マニュアルは**読んで手元に置く物**なので、**1 クリック増えるほうを採った**。
 *
 * **★ なぜ測っていないと書くか**: **この機では測れない**からである
 * （calcarc-1e の判定、2026-09-18）。**`vite preview` は Pages Functions を
 * 通さない**ので、**手元の E2E にこのヘッダは出てこない**——**成立するのは
 * 本番か `wrangler pages dev` の下だけ**である。**「測ってある」と書いていた
 * のを直した**: **measured と inferred を混ぜない**（今日ずっと言っている形）。
 *
 * ## ★ 直ったかどうかは、この機では測れない
 *
