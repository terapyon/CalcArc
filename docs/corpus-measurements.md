...
- `cancellation/band`: **シャード自身が `tolerance` として宣言している
  `abs 5e-10` / `rel 1e-6`**——**私が作る必要すら無かった**
...
**同じ形の番人を `cancellation` にも置いた**——帯の分布
（`severe` 693 / `near_tolerance` 1,307 / `mild` 1）を実データで固定する。
