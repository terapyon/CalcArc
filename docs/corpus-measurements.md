...
## 3 つの穴を埋めたあと——押下台帳を突き合わせた（2026-09-03）

**予測は走行前に固定してあった**（`fix/three-typing-holes` の報告として、
`d0595cb` の時点で監視役に送った）:

| | 予測 | 実測（走行 `33615776300`） |
|---|---|---|
| テスト | 37 | **37 passed**（13.0 分） |
| 打鍵ケース | **1,279 のまま**（標本の上限 100 は変わらない） | **1,279** |
| 指摘 | 0 | **0**（報告書「主張が全部通っている」） |
| 総押下 | **動く。方向と大きさは予測しない** | **19,960 → 19,614。動いた** |
| `pnpm heavy` | 250 passed | **250 passed**（48.3 秒） |

**手動の `Heavy corpus`、`main` の `9d9589e`（穴埋め込み）、両ジョブ success。**
成果物は `gh-ro api "repos/terapyon/CalcArc/actions/artifacts/9841969471/zip"`
で取った（`run download` は断られる——**`api` の zip 経路なら通る**）。

**★ 押下の 19,614 は、この走行の ui 段そのものである。**
報告書は「**この報告書と同時の走行ではない**」と断るが、それは**別プロセス**
という意味であって、別の走行という意味ではない。`heavy-corpus.yml` は
**`pnpm heavy:ui`（:191）と `pnpm heavy`（:214）を同じジョブの続きの段で回し**、
`heavy/heavy-ui-run.json` を受け渡している（このファイルは `.gitignore:26` に
あるのでコミットされていない——**同じ runner の同じ走行でしか読めない**）。
**別の走行の数字が混ざる余地は無い。**

### ★ 数が合った、では足りない。報告書そのものを突き合わせた

**基準は Node 24 の走行 `33356470446`（`2ef7a7b`、押下 19,960）。**
**754 行のうち、違うのは 14 行だけ**である（`<` と `>` が 28 行 = 14 対）。
**14 行とも説明が付く。1 行ずつ数え上げる**:

| 報告書の行 | 変化 | なぜ |
|---:|---|---|
| 23 | 判定表 `scientific` 20020 → **20026** | typed に足した 6 件 |
| 103 | 外部参照 28263 → **28269** | 同上 |
| 120 | 合計 33592 → **33598** | 同上 |
| 126 | 「全 33592 件中」 → **33598** | 同上 |
| 334 | 押下 19960 → **19614** | 標本が入れ替わった |
| 357 | 外部参照の再掲 28263 → **28269** | 同上（103 と同じ数） |
| 392 | 実行日時 | この走行のもの |
| 393 | コミット `2ef7a7b` → **`9d9589e`** | 同上 |
| 394 | wasm `0.6.0` → **`0.7.0`** | 0.7.0 の出荷を挟んでいる |
| 450 | 判定表の `typed` 2000 → **2006** | 足した 6 件 |
| 609 | 相対許容の分布、`rel そのもの` 1998 → **2004** | **6 件とも表示分解能どおりに通った** |
| 655 | 出現回数の `キー列` 2000 → **2006**、`neg` 351 → **354** | **`neg` の +3 は ② の 3 件そのもの** |
| 682 | 括弧の最大深さ、`深さ 0` 407 → **413** | **6 件とも括弧を持たない**（リテラルだから） |
| 708 | 値の分位、件数 2000 → **2006**、最小 1.20e-6 → **1.00e-6**、中央 7.00e+2 → 6.96e+2、最大 9.64e+8 → **9.99e+8** | **★ 下記** |

**★ 分布の両端が動いたのは、私が置いた 2 件そのものである**——
**最小 `1.00e-6` は ② の `1.0e-6`、最大 `9.99e+8` は ① の `999e6`。**
**「分布が動いた」のではなく、置いた値が端に出た。**
**足した 6 件が、報告書の 14 行すべてを説明する。**

### ★ `llm` の 11 件は、この合計に入らない

**+6 であって +17 ではない。** `pnpm heavy` が数えるのは
`corpus/generated/` だけで、**`llm` の golden は `testdata/` にある**。
**同じコミットで足したのに、片方はこの報告書に現れない。**
**「総件数が予測どおり」を、置いた件数の合計と混ぜないこと。**

### ★ この 5 項目で、預けていた宿題は閉じた

**この作業台では `heavy:ui` を 1 度も完走できていない**（3 回とも
Segmentation fault。機械側だと
[known-flaky-tests.md](superpowers/sdd/known-flaky-tests.md) で確定済み）。
...
**【追記 2026-09-11】その後、CI を通った**——Heavy corpus 走行 `34543925445`
（`heavy/finance-start` の `e10f82a` を手動起動、ジョブ `103092418248`）。数は節の終わりの
「CI で確かめた」に書いた。
...
**【追記 2026-09-11】この表は CI（走行 `34543925445`）の `heavy-report.md` の検出の表と
1 件も違わない**（節の終わりの「CI で確かめた」）。
...
**【追記 2026-09-11】CI の走行番号は `34543925445`（ジョブ `103092418248`）。** 下の節。

### CI で確かめた（2026-09-11、Heavy corpus 走行 `34543925445`）

`heavy/finance-start` の `e10f82a` を手動起動（ジョブ `Corpus vs reference` =
`103092418248`、成功）。数はその走行の成果物 `heavy-report.md` と、ジョブのログ・
ステップの時刻から写した。

- **検出の件数は、上の表と 1 件も違わない。** 期首 951 / 108 / 969 / 640 / 594 / 459 / 305、
  `finance-000.json` は 11 行とも同じ。`finance-000.json` の検出数は v0.9.0 の Release 走行
  `34479446474` の報告書とも 10 本すべて同じ（新しい変異は v0.9.0 に無い）。**期首の
  `minRate` は余裕 0 のまま通った**——検出は決定的で、手元と CI は一致する
  （09-10 の 910 に続く 2 例目）
- **`heavy:ui` は 40 passed（12.0 分）**（v0.9.0 は 37 passed / 11.9 分）。増えた 3 件が期首の
  3 件で、**最初の実走で緑**: `compound_grow` `fin-start-000545`・`compound_deposit_for`
  `fin-start-000526`・`compound_periods_for` `fin-start-000713`。既存の面ごとの 16 件は
  ケースの id が v0.9.0 の走行と同じ
- **`Corpus vs reference` は 37 分 08 秒**（制限 85 分）。v0.9.0 の 35 分 36 秒より 92 秒長く、
  うち `Measure what this corpus can detect` が +68 秒（20:54 → 22:02）、`Type a sample on
  the real keypad` が +6 秒（11:58 → 12:04）、`Run the heavy corpus` は 1:01 のまま
  （257 → 263 passed）
