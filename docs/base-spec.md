...
**【2026-09-10】** **1.0 からは、一覧に挙げた画面の URL を約束する**——以後の版でも
同じ画面を開く（[1.0 の門の設計](superpowers/specs/2026-09-10-one-point-oh-gate-design.md)
§2.3、一覧は `web/tests/promised-urls.ts`）。**ここに数を書かない**——
**0.9.6 で 13 から 14 になり**（`#manual` を足した）、**この行だけが古いまま残った**。
**数は一覧が持ち、下限は `web/src/route.test.ts` が見張る**（**書かなければ腐らない**）。上の「互換は作らない」は
いずれも 1.0 までの判断である。1.0 のあとにどれかを消す・綴りを変えるなら、
古い URL を新しい画面へ向ける分岐を足す。
...
**履歴が最初に出たのは 0.8.0 である**（`git tag --contains` で、`History` の画面を
足したコミットを含む最初のタグが `v0.8.0`）。
...
**いま保存しているのは 9 項目**——角度の単位・極形式（Scientific）、
データ型・主表示の単位系（Data Scale）、計算の種類・期数・源泉徴収・
積立の時点（Finance）、履歴を残すか。**7 と書いてあったのは 0.9.x の
どこかで腐った**（積立の時点と履歴が足されたのに、この行が動かなかった）。
数えた先は `web/src/settings/index.ts` の `parse`、外した理由は
...
> **【2026-09-24】この但し書きは役目を終えた。** `0.3.1` は**出荷済み**で
> （タグ `v0.3.1` が在る）、**為替換算は本番で動いている**。
> 以下は、書いた当時（2026-08-20、まだ出る前）の断りである
> ——**`Cargo.toml` に `0.3.0` とあってもこの節と食い違わない**、という意味だった。
...
- **例外が 3 つある**（関数列の高さ 34px、**Finance の項目行 34px**、フッタの
  ボタン）。**Finance の項目行は 2026-09-10 に例外へ戻った**（`definition-of-done.md`
  の表が正。**2026-09-24 に実行役が現物の表を数えた——3 行**）。どれも理由つきで
