...
配信、**パネルが出てから測る**）。Convert は 3 カテゴリで盤面を 1 つ共有する
（`UnitPanel`）ので、**カテゴリを替えても縦は 1px も動かない**:
...
**3 行が同じ値なのは偶然ではない。** 枠の高さは中身の行数ではなく
`UnitPanel.module.css` の `aspect-ratio: 1 / 1` が決めており、単位が 11 個でも
...
通り、実ブラウザで単位面が length 366×291 / mass 366×216 / **温度 366×67** に
潰れることを、この検査の赤確認で実測した）。
...
**つまり「巡回に入っていない route」は 0.3.0 の終わりでも 2 つ残る。**
