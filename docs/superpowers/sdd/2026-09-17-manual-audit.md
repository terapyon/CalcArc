...
| 2 | `:737`「年利は 8 文字」 | **マニュアルが偽** | `MAX_RATE_LEN` は木全体で 2 箇所のみ（定義 `FinancePanel.tsx:147`、`pushDot` `:662`）。`pressDigit` は項目で分岐せず `pushDigit(…, MAX_YEN_DIGITS = 20)`。**20 桁打てる**。100.0000 は計算時の `SyntaxError`（`loan/rate.rs:42-44,66-69`） |
...
**高さの検査はエンジンで分岐するものがある。** `finance-layout.spec.ts:183-195` の「複利の面の余白」は高さの検査（`mainHeight - panelHeight >= 8`）であり、`:187` `const narrow = size.width === 360;` → `:195` `setViewportSize(narrow ? narrowSize(browserName) : size)` で、**WebKit だけ 375×812、Chromium は 360×800** で測る（`widths.ts` の `WEBKIT_NARROW_HEIGHT = 812`）。
...
- 「**高さの検査に WebKit 固有のものは無い**」は**誤り**。`finance-layout.spec.ts:187,195` の複利の面の余白は高さの検査で、WebKit だけ 375×812 に差し替わる。
