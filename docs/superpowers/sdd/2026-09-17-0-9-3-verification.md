...
**行き先は `engine_table.rs` の `an_unmatched_closing_paren_cannot_be_pressed`、D-2 の枝 `fix/unmatched-rparen`。**

**最初は「移す予定」と書いたが、確かめたら既に在った**——`111e67f` の **1328〜1329 行**に
`refused_after(&[], "rparen")` と `refused_after(&["3", "add", "4"], "rparen")` が並んでおり、
**退役させた 2 件と 1 対 1 で対応する**（**その commit を読んで確かめた**。
3d は `refused_after(&["lparen", "3", "rparen"], "rparen")` も足していて、
**閉じ終わった組のあとの `)` は、私が退役させた 2 件が覆っていなかった形**である）。
**註を SHA で名指しに書き換えた**（監視役の指示のとおり）。

**ただし「D-2 が仕上がった」とは書かない**——**`111e67f` は先端が赤い**。
**2 行が在ることと、枝が緑であることは別**であり、**今日の事実は「置き場が決まった」までである。**
...
## `pnpm heavy` 2 回目（再生成後） — **赤 1 本 → 直して緑**

**1 回目が緑だったので、この赤は自分の変更である**と即座に切り分けられた——**1 回目を先に回した狙いが、そのまま効いた。**

### 鳴ったもの

`heavy/tests/corpus/report.spec.ts` の
`the in-progress display item is counted from the corpus, not written by hand`:
**`"errors-000.json (displays)": 19` に対し実測 17**（−2）。

**退役させた 2 件は、どちらも `=` を押さないキー列だった**ので、
**「`=` を一度も押さないキー列」の数に入っていた。**
**「キーを拒むと床が全部動く」の実例**——**エラー種別の 2 件を抜くと、
種別とは無関係な「`=` の有無」を数える床が動く。**

### ★ 同じ数が 2 か所に手で書かれていた

**19 → 17 に直したら、こんどは別の行が鳴った**——
`:1824` の `expect(markdown).toContain("`entry-000` 35 本・`errors-000` 19 本")`。
**内訳の表と、レンダリング後の本文と、2 か所に同じ数が焼いてあった。**

**「直す前に差分を数える」を 1 回目は守れていなかった。**
2 本目が鳴ったあとに `heavy/tests` と `web/tests` を grep して**残りを数えてから**直した
（**結果は 1 件のみ**で、他の `errors-000` の言及は散文で数を持たない）。

### ★ 数が動いただけでなく、説明が正確になった

内訳の註は **「単項関数が `=` を待たずにその場で撥ねるケース」** と書いてあったが、
**19 のうち 2 件は単項関数ではなく括弧の構文エラー**だった
——**この説明は 19 件中 17 件しか説明していなかった。**
退役後の 17 件を**全件印字して確かめた**ところ、
`recip` / `ln` / `log10` / `sqrt` / `asin` / `acos` / `tan` / `n_fact` の**単項関数だけ**である。
**退役によって、註と実物が初めて一致した。**

### 報告書の外向きの主張は、直す必要が無かった

テストの註は**「ここが動いたら、まず直すのはこの数字ではなく報告書が外の読み手にしている主張のほう」**
と命じている。**確かめたところ、その主張は算出されている**——
`heavy-report.md:321` は `- **入力中の表示——52 本のキー列が `=` を一度も押さない。**` と
**走行が数えた値で書き出されており**、54 → 52 も内訳 35/17 も**自動で追随した**。
**手で書かれていたのはテスト側の 2 か所だけ**だった。

### もう 1 つ腐っていた理由（生成器の側）

`reference/src/calcarc_reference/corpus_combinatorics.py:12` が
**「`errors-000.json` は全領域の寄せ集め 30 件である。除算・…・値域・括弧が 1 枚に同居している」**
と**現在形で**書いていた。**件数も、並べた領域（括弧）も、両方が偽になった。**
**28 件に直し、括弧が退役したことを括弧書きで残した**——
**この節の論旨（「足りなかったのは件数ではなく場所と体系性」）は件数に依らない**ので、論旨は動かしていない。

**Ruling: 過去の記録は書き換えない**
— `docs/heavy-corpus-implementation-report.md:458` と
`docs/superpowers/sdd/heavy-HANDOFF-de.md:53` にも「30 件」が在るが、
**どちらも「その段が何を納めたか」を SHA と対で記した過去の記録**である。
**納品の記録を後から書き換えるほうが害が大きい**ので、そのままにした。
— **コストが外れた場合**: 将来の読み手が古い 30 を現在の数として引く危険が残る。
**「理由は静かに腐る」は 3 段伝播する**ので、**もし引かれたら、そのときは記録の側に日付を足す。**

### 緑の印字（4 回目の走行）

**267 passed（41.8s）。** `pnpm lint`（heavy + tools）緑、`tsc --noEmit` 緑、
`pnpm test` **21 files / 364 tests**。`reference` **547 passed** / ruff 2 つとも緑 / **`uv.lock` 無変更**。
（`biome` の info 2 件は `tests/corpus/report.ts:1690,1700` の `useTemplate` で、
**私が触っていないファイル**・**exit 0**。）
