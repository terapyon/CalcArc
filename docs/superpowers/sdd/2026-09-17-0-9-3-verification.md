...
**解消済み（2026-09-17、`2ccd196`）。** 枝 6（`chore/false-rationales`）を合流したあと、
**年利の 3 行を足した**（上限 100・小数 4 桁・5 桁以上）。**待ちは残っていない。**
...
## C-1 の番人 — **上限 10 行 + worked example 6 行 + 型の上限 1 行（待ちは無い）**
...
**主張は消えたのではなく、移った**——D-2 が `)` を押せなくすると
...
### ★ 直す文と、直さない文の線（2026-09-17 に確定）

**今日 1 日、腐った理由を何件も直した。その全部に共通する線はこれである:**

| | 例 | どうするか |
|---|---|---|
| **生きている註** — **現在形で「いま何が在るか」を語る** | 「`errors-000.json` は寄せ集め**である**」「単項関数が撥ねるケース**である**」「プールは括弧の経路を**含む**」 | **直す。** 偽になった瞬間に、読んだ人を誤らせる |
| **過去の記録** — **過去形で「そのとき何を納めたか」を語る**（多くは SHA・日付と対になっている） | 「`4a6347c` + `2ce03a4` — `errors-000.json` **30 件**」「その段で**追加された**」 | **直さない。** 書き換えると、納品の記録そのものが嘘になる |

**判別は時制ではなく「何を主張しているか」で決める**——**「いまの姿」の主張か、「あの日の出来事」の主張か。**
**迷ったら、その文に日付か SHA が対になっているかを見る**（対になっていれば、たいてい記録のほうである）。

**この線が引けないと、両方向に間違える**——**記録を書き換えて履歴を失う**か、
**生きた註を「記録だから」と言って腐らせる**か。**今日は後者を 4 件直した。**
...
## C-7 `heavy:ui` が `v0.9.3` のタグで落ちた — **直して緑**（2026-09-18）

**枝** `fix/reachability-precondition` = `d6699f6`（`origin/main` = `4aad64d` から 1 本）。
**C-1〜C-6 の枝は既に main に入っている**ので、**その上には積まず切り直した。**

### 何が落ちたか

```
tests/ui/reachability.spec.ts:62 › every button the keypad claims can actually be pressed
  + "rparen: \"閉じ括弧\" は在るが無効化されている(押せない)"
```

**D-2 の帰結である。** `)` は**開いている組が無いときだけ**押せなくなったが、
この検査は**初期状態しか見ていなかった**。

**★ なぜタグまで見えなかったか**——**`heavy:ui` は毎回の CI では走らない。**
**D-2 の 11 本の赤を数えたときにも、末尾のスイープでも出ない。**
**番人が走る段が違うと、タグの 40 分の走行が最初の告知になる**（#144 と同じ形）。

### 数えた（1 本鳴ったのは標本にすぎない）

| 場所 | 判定 |
|---|---|
| `reachability.spec.ts` 第 1 面の `isEnabled` | **赤**。`)` は第 1 面（裏は `n_p_r`） |
| 同 Shift 面の `isEnabled` | 影響なし（`)` は `needsShift` でない）。**同じ形**なので将来は鳴りうる |
| `presses.ts` の `auditPresses`（`never-pressed`） | 鳴らない。`)` は**対になった形で**押され続ける |
| `corpus-ui.spec.ts` | 鳴らない（根拠は下） |
| `finance-ui.spec.ts` | 影響なし。Finance の `(` `)` は**文字を入れるキー**で、`refused` を読むのは `ScientificPanel` だけ |

**科学盤面以外の「常に無効な 7 つの演算子」は無関係**——あれは Data Scale / LLM / Transfer で、
この検査が見るのは `SCIENTIFIC_SECTIONS` だけである。**`)` は科学盤面で初めて「初期状態で無効」になったトークン。**

### 直した形——**一覧から外さない**

**「押せないキーを一覧から外す」は、「盤面が持つと言っているボタンは押せる」という主張を捨てることになる。**
`)` は**いつでも押せない**のではなく**開いている組が無いときだけ**押せないので、**前提を満たしてから押す。**

**トークン → 前置き**の表（`PRECONDITION`）を置き、**飛ばさずに**前置きを押してから判定する。
そのうえで**テストを 1 本足し、3 点で主張する**——**前は無効 → 後は有効 → `ac` で無効に戻る。**

**1 点目が表の腐りを止める。** これが無いと、**`)` が「いつでも押せる」に戻った日も緑**になる
（前置きを押しても押さなくても押せるので、**表だけが残って空回りする**）。
**3 点とも同じ `isEnabled()` で読む**——片方を別経路にすると
**`disabled` と `aria-disabled` の食い違いを見逃す**（`Key.tsx:90-91` は両方を同時に付ける。3d の指摘）。

**前置きは `["lparen", "3"]`。** `["lparen"]` だけでも足りるが、
**その列が `engine_table.rs` に `accepted_after(&["lparen", "3"], "rparen")` として在る**
——**盤面の検査の前提が、engine の仕様表に固定される。**

### ★ 私の読みの穴（3d が潰した）

私は条件を**「深さ > 0」**と読んでいた。**1 点で嘘になる**——
**`refuses` はエラー中に全部 false を返す**のに、**`display.rs:88` の `pending_depth` はエラー中を 0 に潰す**。
**つまりエラー中は「深さ 0 なのに `)` は押せる」。**
盤面の検査は踏まないが、**註に書いていれば偽の理由が 1 つ増えていた。**
**3d の `refuses` の抜粋をそのまま貼り、要約していない。**

### ★ engine 側に穴が 1 つあった（見つけて移した）

**3 点目（`ac` のあと無効に戻る）だけ、`engine_table.rs` に対応する行が無かった。**
真であることは `d7274a6` の使い捨て作業木に probe を足して測った（2 本とも緑。そのあと `git restore` → `worktree remove`）。

**真なのに主張がどこにも無い**＝**私の盤面テストが唯一の見張り**になる、しかも**12.9 分・タグでしか回らない段**に。
**#144 とまったく同じ配置**なので、**3d に engine 側へ移してもらった**——
`test/ac-clears-the-open-group` = `61c4b71`（変異で名指しの赤も取得済み）。**いまは `cargo test` の数秒で鳴る。**

### ★ 偽の赤を報告しかけた（新しい作業木の罠、今日 2 度目の形）

切り直した作業木で `playwright --list` が **`Total: 0 tests in 0 files`** を返した。
**spec が消えたのではない**——**`web/src/wasm/` が無い**だけで、
`Cannot find module '../wasm/calcarc_wasm.js'` が**「0 件」に化けていた**。
**`pnpm wasm`（重い段）は回さず、`wt-093` からビルド済みを写した**（gitignore 済みで差分に出ない）。

**気づけたのは数字が極端だったから**（41 → 0）。
**42 → 41 のような動き方なら、たぶん見落として「1 本減った」と報告していた。**
**`not found` は赤ではなく「0 件」という穏やかな顔で出ることがある。**

### 緑（**走らせたのは 3d。私の機では `heavy:ui` は segfault で走らない**）

`4aad64d` ＋ `d6699f6` ＋ `61c4b71` を合流した使い捨ての木で**全走**——**42 本すべて緑・12.8 分・exit 0**。

- 足した 1 本 `a key with a precondition is refused before it and accepted after` **緑**
- タグで落ちた `every button the keypad claims can actually be pressed` **緑**（題は同じ、行が :62 → :134 に動いただけ）

**私の側で示せたのはここまで**: `tsc` 緑・`pnpm lint`（heavy + tools）緑・`pnpm test` 緑・
**`playwright --list` が 42 件を収集**。**「収集される」は「緑になった」ではない**ので、分けて書く。

### ★ 「C-6 のおかげ」が、実走で裏づいた

私は当初**「C-6 で対応しない `)` を外したから `corpus-ui` が鳴らない」と推論**で書いた。
3d が**静的な根拠に差し替えた**——**`corpus_refused_presses.rs` と `loadShards()` は同じディレクトリを読む**
ので、**一回きりの事実ではなく続く保証**である（**シャードの置き場が分かれたら、この結論は崩れる**）。

**さらに実走の台帳が数で裏づけた**（`heavy-ui-run.json`、3d の走行）:
**`rparen` を実ブラウザで 1,179 回押して、1 度も無効なボタンに当たっていない**
——当たれば actionability 待ちで TimeoutError になる。
`lparen` は 1,184 回で **5 多い**（**閉じ忘れた組を `=` が畳む列**があるため。辻褄も合う）。
**推論 → 静的な保証 → 実走の数、と 3 段で固まった。**
