...
**★ 同じ形を、この節を書いた側が同じ文書群で踏んだ**（2026-09-25、外部指摘 M9）。
**0.9.6 の総ざらいで、英語版（`quick.en.md`）は「マニュアル画面の記述が在るか」で見て通した**
——**在ったので通した。ところが冒頭の導線は 0.9.5 以前のままだった**（日本語 2 冊は 0.9.6 で
直してある）。**「在る」と「正しい位置に在る」を分けた節を書きながら、別の文書で分けなかった。**

**発注の形が、そのまま漏れの形になった**（監視役の併記）——**あのとき聞いたのは
「英語版にマニュアル画面の記述が在るか」**で、**「英語も日本語と同じ構成か」ではなかった。**
**問いの形が答えの形を決める**（§0.6 と同じ）。**次に翻訳の遅れを見るときは、
「同じ節が同じ順で在るか」を問うこと**——**語の有無では見えない。**
...
## 3.5 外部の指摘から持ち越したもの（2026-09-25）

**マニュアルの外部指摘 9 件（M1〜M9）のうち、M8 だけがマニュアルの話ではなかった。**

- **M8「状態保持」**（指摘者の番号は **F18**。**この綴りはリポジトリに 0 件**で、
  指摘者の側の採番である）。**マニュアルの直しには入れない**（利用者の裁定 2026-09-25 で
  M1〜M9 の文は 0.9.8 に入るが、これは文の話ではない）。**持ち越し。**
  **現物（`849d432` で確かめた）**: 端末に残るのは **3 つだけ**——設定 9 項目
  （`calcarc.settings`）・履歴（`calcarc.history`）・為替レートの塊
  （IndexedDB の `calcarc-currency`）。**打ちかけの入力と表示の状態は保存していない**
  （`EngineState` を `localStorage` に書く箇所は 0 件）。**版が変わると設定と履歴は
  失われうる**——**1.0 では「約束しないこと」に書く**（門 2 の設計書 §3 の文案）。
  **大きさ**: 中（何を保持するかは製品の裁定であって、検査の穴ではない）。
