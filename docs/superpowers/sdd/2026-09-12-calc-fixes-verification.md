...
- ~~Task 4: wasm32 のテストは `max_monthly_yen` を確かめない（native はする）。~~ **2026-09-16 に閉じた**
  （PR D・枝 `heavy/loan-cap-guard`、`91c243a`）——`crates/calcarc-wasm/tests/loan_boundary.rs` に `want <=
  cap` / `exempt` 行は `want > cap` のアサートを足し、native と同じ形にした（Task 2）。
...
- ~~(0.0001%, 2) の B の上位 10%（≈ 5.4e15〜6.0e15）に golden の件が無い。~~ **(a) は 2026-09-16 に閉じた**
  （PR D・枝 `heavy/loan-cap-guard`、`91c243a`）——残価の上位の段（B の上限の 95%・99%）を `_top_residual_js`
  で足し、(0.0001%, 2) の最大 B は 5,940,000,230,999,978（約 6.0e15 の 99%）まで伸びた（設計書 §4.7）。
- n ≥ 12 の残価の件が 0（c2 の分母が上限から決まる `b_limit` を超えるため、§3 の表）。**これは開いたまま**
  ——PR D は (a) だけを閉じ、この限度には触れていない（名前つきの持ち越し、§6 の ruling 129）。

**PR C・PR D**（計画の「残り（別の計画）」より）:

- **PR C（F1 のシャード、設計書 §3）。** calcarc-3d のエンジン修正の枝（`fix/engine-correction`）ができてから、
  その上に積む計画を書く。**2026-09-16: 枝 `heavy/operator-correction` として実装済み**（下）。
- **PR D（10 億円を超える月額の `Overflow` の見張り、§4.8）。** calcarc-3d の上限の実装の上に積む。コアの
  定数と JSON の `max_monthly_yen` の一致の検査もここ。**未着手のまま**。

**PR C（`heavy/operator-correction`）で閉じた項目（2026-09-16）:**

- **`neg`（`entry-000033`）。** 下の「F5 の『関数の答えの後』と `neg`」の 2 つの論点のうち、`neg` 側は閉じた。
  R4 の裁定でキー列を `1 2 neg 3` → `1 2 neg add 3 eq`（表示 `-9`）に打ち直し、拒まれる押下を作らないようにした。
  R5 で、コミット済みの全コーパス（35,391 列）に製品の `engine::refuses` が拒む押下が 0 本であることを
  `crates/calcarc-core/tests/corpus_refused_presses.rs` が恒常的に見張る。**「関数の答えの後」の側（product 側の
  F5 の設計、calcarc-3d §3.7・§9）はこの branch の範囲外で、下の「利用者の裁定待ち」に残す。**
- **設計書 §3.10 の「266」の訂正。** 表の 244／22（計 266）は controller が規則を再実装して数えた誤りだった。
  製品自身の `engine::refuses` で数え直すと、このブランチの前は 1 本（`entry-000033`）、後は 0 本
  （設計書 §3.10【訂正 2026-09-16】）。
- **設計書 §6.2 の実装の形の訂正。** 「表と `areaOfShard` の一覧が食い違ったら赤くなる単体テスト」という文を、
  実装どおり（`comparisonOf(area: Area)` の型と `areaOfShard` が未知の接頭辞を拒むこと。既存の単体テストは
  それとは別の・より強い性質——厳密の領域が相対誤差を測ったら報告書を落とすこと——を見る）に書き直した
  （calcarc-1e の注記 B、設計書 §6.2【訂正 2026-09-16】）。
- **設計書 §3.3／§3.6 の「480 セル」の訂正。** 格子は 348 セル（除外 132 セル）で、480 ではない
  （設計書 §3.3【訂正 2026-09-16】）。台帳としては独立の論点ではないが、§3.10 の訂正と同じコミットで直した
  ので、ここに記録する。

**利用者の裁定待ち:**

- ~~10 億円の上限を年利 0%・1 回払いにも掛けるか（数値方針の新節の範囲の文を、裁定後に書き足す）。~~
  **裁定済み（2026-09-13）**——掛けない。数値方針 `:177` の後の文と設計書 §4.7・§4.8 が範囲を書く
  （PR D・枝 `heavy/loan-cap-guard`、2026-09-16）。
- F5 の「関数の答えの後」（`neg` 側は上のとおり閉じた。calcarc-3d §3.7・§9 の範囲）。

## §9 PR D（上限の見張り）が足した行の内訳

`heavy/loan-cap-guard`（91c243a、Task 2）が golden に足した 156 件（`over_cap` 15・`exempt` 13・残価の
上位の段 128、計 2,526 → 2,682）のうち、`over_cap`・`exempt` の内訳は次のとおり（実装当時の作業場の記録
から転記した。作業場は追跡外で、この計画が閉じれば消える）:

- **`over_cap`（15 件）を op 別に見ると**: `loan_forward` が 12 件（うち残価ありが 4 件）、
  `loan_bonus_forward` が 3 件。
- **`exempt`（13 件）を経路別に見ると**: 年利 0% が 9 件（うち残価ありが 2 件、1 件は元本
  `principal = u64::MAX`・`n = 600` → `30,744,573,456,182,586`——製品自身の単体テスト
  `the_exact_paths_are_outside_the_cap` と同じ入力）、1 回払い（月利の両端）が 4 件。
- 上限超えの行は**上限 + 1,000 円と上限 + 1,000,000 円**の 2 段に置かれている（`loan_boundary.py` の
  `OVER_CAP_MARGINS_YEN = (10**3, 10**6)`）。R2 の下限（上限 + 2 円）はテストが理論値を作り直して別に見る
  ——`+1,000` はそれとは別に、この golden が実際に置いている行のマージンである。

（§4.8 の「なぜ上限超えの行を上限 + 1,000 円に置くか」の根拠そのものは設計書側に書いてあるので、ここは
行の内訳だけを持つ。）

## §10 PR D（上限の見張り、枝 `heavy/loan-cap-guard`）の最終レビュー・修正の波・裁定

この節は 2026-09-16 の PR D の締めの記録である。§7 の「最終レビュー」は PR A・B のもの（`fb32bc6` の頃）で、
別物である。PR D の作業場の記録（最終レビュー・報告・台帳）は**追跡外**で、この計画が閉じれば消えるので、
引く価値のあるものをここへ移す。

### §10.1 最終レビュー（opus、`aa1f7e0`、5 コミット、起点 `ba46116`）

**判定: With fixes。** Critical 0・Important 2・Minor 5。

- **I-1.** 設計書 `:355`・`:432` が `docs/superpowers/sdd/2026-09-16-loan-cap-guard/task-*-report.md` を引くが、
  **その場所は存在しない**（報告は追跡外の作業場にある）。`check-citations.mjs` は base-spec の引用しか見ない。
  ——**押し直しの枝で一度直したのと同じ形の欠陥**。
- **I-2.** §4.8 と計画の R1 は「golden が上限を両側から挟む」と書くが、**下の顎は 1 行だけ**
  （`residual/exact/15/2/987654400/81`）で、**その行の存在を要求するテストが無かった**
  （`test_loan_boundary.py:70,72` は `<= cap` と `>= cap//2` しか見ない）。上の顎の約 1,000 円の死角も
  追跡下の文書に書いていなかった。
- **Minor 5 件.** PEP 758 の `except` の綴り（`test_loan_boundary.py:215`）／`test_loan_ref.py:185` の上限の
  即値／+1,000 を主張しながら +2 しか assert しない／git が無いと黙って `skip` する番人／§4.7 の「94%」が
  実測 93.60%。

**レビュー役が一次資料で確認した「正しい」側**（この枝の主張のうち、独立に裏が取れたもの）:

- 番人は**両方向・両ハーネスで実際に効く**（`over_cap/over` errors 15・`exempt/over` same 13 が native と
  wasm32 で一致）。
- **Rust のテストに上限の即値は無く**、native も wasm32 も JSON の `max_monthly_yen` を読む
  （wasm32 側は今回が初——§8 の持ち越し「wasm32 は `max_monthly_yen` を確かめない」を閉じた）。
- 2,526 → 2,682 で**欠落 0・変更 0**、相対順序も保存。`独立:` の宣言は無傷。
- +1,000 円の余裕の算術が正しい（ε/(2r)·1e9 = 1.3323 円）。文書の 13 の数が一次資料と一致
  （§4.7 の native の 11 セルは `cargo test` で再現）。

### §10.2 修正の波（`b858500`、1 コミット、文書 2 本と `reference/tests/` 2 本のみ）

- **I-1:** §4.7 の引用は落とし（中身はその場に在った）、§4.8 の引用先は**この台帳の §9 に写経して**付け替えた。
  `git grep -nE "task-[0-9]+-(report|brief)\.md|\.superpowers/" -- docs reference crates` は、この枝が足した
  ヒット 0。
- **I-2:** `reference/tests/test_loan_boundary.py` に
  `max(floors) == loan_ref.MAX_VERIFIED_MONTHLY_YEN` を足し、§4.8 に死角の一文を足した。
- **Minor:** 上限の即値を `loan_ref.MAX_VERIFIED_MONTHLY_YEN` から導出／主張どおり +1,000 を assert（R2 の
  +2 の床は**残した**——別のものを守る）／git が無いとき `pytest.fail`（`skip` をやめた）／§4.7 を 93.60% に。
- **`except` の綴りは直していない**（§10.4 の裁定）。

### §10.3 限定再レビュー（sonnet、`aa1f7e0..b858500`）
...
- 空振りの assert・テストコードに書かれた許容や上限・実測から離れた文書の数は、いずれも無し。
- `cd reference && uv run --no-config pytest -q`: **521 passed**。

### §10.4 裁定（18 件、出た順。それぞれ「決めたこと — なぜ — 誤っていたときの代償」）

**Pre-flight / 計画の裁定（R1〜R5）**

- R1 コアの定数は非公開なので、上限ちょうどの行と上限超えの行で振る舞いを挟んで縛る — 製品を公開に変える
  ことを頼まない — 誤れば定数の食い違いが別の形で出る（どちらかの行が赤）。
- R2 上限超えは理論 ≥ 上限 + 2 — (上限, 上限+1) は f64 で両側に出る — 誤れば libm の 1 ulp で赤が揺れる。
- R3 0%・1 回払いの対象外の行で範囲を見張る — 利用者の裁定 09-13 — 誤れば範囲の変更が golden で見えない。
- R4 上位の段は足すだけ、既存 2,526 件は部分集合でバイト同一 — 既に承認・測定済みの件を動かさない —
  誤れば §4.7 の比べ先が消える。
- R5 参照にも上限、コーパスは動かない（`PRINCIPAL_MAX` 5 億、`1e` の読み）— §4.8 — 誤れば再生成の検査が
  赤くなって気づく。

**実装中の裁定**

- 逸脱「上限の判定を `_guard_boundary` の**前**に置く」を認める。前提を私も確かめた——`_guard_boundary` の
  限度は `max(GUARD_ABSOLUTE, |value| × GUARD_RELATIVE)` で、5e8 で 0.5（整数までの距離の最大値）に達するので、
  **5e8 以上はすべて境界近接で棄却される**。したがって後ろに置いた上限は永久に届かない — 簡約書の「後に置く」は
  私の書き損じ — 誤りなら 5e8 未満の挙動が変わるが、そこは guard が先に効くので変わらない。
- 賞与のテストの値を 200 億/90 億 → 60 億/30 億 に差し替えた逸脱も認める（元の組は月払いの脚が先に guard に
  掛かる）。**賞与の脚の上限を踏んでいるか**はレビューで確かめる。
- レビュー役の Important（guard の実質 5e8 の天井を `numerical-policy` に書く）は**置き場を変えて採る**——
  `numerical-policy` は製品の規則を書く場所で、guard は参照側の「golden の選び方」の道具。設計書 §4.8 に書く —
  誤れば公開の文書に参照側の事情が混ざる。
- Minor（wasm32 のテストが誤ったエラーコードを `wanted {code}` 無しで印字する。native は付ける）は**直す** —
  失敗の診断は失敗したときにしか読めないので、そのとき足りないと困る。
- 上の直しは失敗の経路の書式だけだが、**枝の最後に `wasm-pack` をもう 1 回回す**（「試したバイナリ＝
  コミットしたバイナリ」を保つ）。重い段なので監視役と調整する — 誤りでも数分の損。
- Minor（Step 2 の再生成前の赤「`over_cap/over` にケースが無い」が報告に無い）は受ける——赤 (b) が同じ向きを
  覆っている。
- Minor（HEAD 比較のテストの下限が 2,500 で 2,526 を導けない）はレビュー役の判断どおり受ける。
- 引用の行番号の Minor は**私が直す**（1 行。行番号は現物の印字から取る）。そのあと枝の最後の `wasm-pack` を
  回す。
- `pnpm install --frozen-lockfile`（web・heavy）を入れて**同じ先端でスイープを回し直す** — 誤りなら 4 分の損。

**最終レビューを受けての裁定**

- 1 回の修正の波で I-1・I-2 と Minor 4 件を直す。**PEP 758 の `except` の綴りは直さない**（`ruff` の出力で、
  Python 3.14 に固定されている）— 誤りでも文とテストの 1 行ずつ。
- 修正の波にも**範囲を絞った再レビューを付ける**（1 波 → 1 再レビュー）。最終レビューに答える差分こそ、
  誰にも見られずに入りやすい — 誤れば再レビュー 1 回分の手数が無駄になるが、省けば最後の変更が無検査で
  PR に乗る。
- 先端 `b858500` で**全スイープは回し直さない**（根拠は §10.5）— 誤れば入力の変わっていない段を回し直さな
  かっただけだが、もし差分に Rust か生成物が混じっていれば見落としになる。**だから先に名前の一覧を印字して
  数えた**。
- 枝は**そのまま置く**（`finishing-a-development-branch` の選択肢 3）。push と PR の作成は利用者の手番という
  立っている指示があるので、統合の献立は出さない。作業木も残す — 誤れば利用者が自分で消す手数が増える。

### §10.5 検査の記録

- **全スイープは `aa1f7e0` で ALL-OK**: cargo の ok 行 23・web 542・heavy 316・`pnpm heavy` 266・
  reference 521・作業木は空・`uv.lock` 無変更。
- **wasm32（`aa1f7e0`）**: crate 全体 49 passed。`over_cap/over` errors 15・`exempt/over` same 13 で
  native と一致。ドライバは機体の Chrome 135 に合わせて `--chromedriver` で明示
  （環境変数 `CHROMEDRIVER` は効かない）。
- **`b858500` で回し直していない理由**: `git diff --name-only aa1f7e0..b858500` が 4 本——文書 2 本と
  `reference/tests/` 2 本だけで、Rust・web・heavy・生成物は 1 バイトも動いていない。その 4 本を覆う門
  （reference の pytest 521・ruff・format・mypy・`check-citations`・`cargo fmt`＋`clippy -D warnings`）は
  **先端で 2 回、別々の役が回して緑**である。

## §11 PR C（押し直しのコーパス、枝 `heavy/operator-correction`）の最終レビュー・修正の波・裁定

この節は 2026-09-16 の PR C の締めの記録である。§7 は PR A・B の、§10 は PR D の最終レビューで、いずれも
別物である。PR C の作業場（最終レビュー・タスクごとの報告・台帳）は**追跡外**で、この計画が閉じれば消える
ので、引く価値のあるものをここへ移す。

### §11.1 最終レビュー（opus、`4b4b1d2`、11 コミット、起点 `4499f05`）

**判定: With fixes。** Critical 0・Important 2・Minor 4。

**この枝の中心の主張が、独立に裏づけられた。** レビュー役は**自前の shunting-yard を厳密な有理数で書いて
2,000 件の期待値をすべて再導出**し、**食い違い 0・最大相対誤差 1.6e-16** を得た。エンジン由来の
`BINARY_PRECEDENCE` の参照が 0 回であることも確認している。「期待値はエンジンの移植ではない」は、
議論ではなく実測で立っている。

同じレビューが一次資料で確認した他のもの:

- 格子の到達可能なセルはちょうど **348**。
- `same-level` の 361 件は、2 通りの読みが**木として同一である**ことを証明できる（だから赤くならない）。
- 修正前のエンジンで赤くなる 454 件は**全件が `discriminating`**（`same-level` 0・`control` 0）。
  したがって 639 − 454 = **185** は厳密な数である。
- 検出力 454 / 656 / 726 / 572 と、既存 19 本の変異の非退行。
- 番人は実際に走る（35,391 列・床 35,000・製品の `refuses` を import・`keys`/`left`/`right` が唯一の
  列の場であることの証明つき）。
- `entry-000033` → `-9` は id と順序を保って正しい。

**Important 2 件:**

- **I-1.** 引用が主張を支えていない。`corpus_opcorr.py:6,19` ほかが「読んだのは `base-spec.md:333-343` だけ」と
  書くが、**その範囲にあるのは `xʸ` の右結合と `nPr`/`nCr` の位置だけで、「`+ −` より `× ÷` が強い」は
  `docs/numerical-policy.md:673` にしか無い**（修正の波で現物を確認した）。独立性そのものは無傷だが、
  支えない引用は偽の理由が写経で伝播する経路である。
- **I-2.** **追跡下の計画が直っていなかった**——`:136`/`:144` が格子 480 のまま（裁定 E は 348 + 132 で、
  設計書だけ直していた）。`:48` の `engine_table.rs:1188` も実際は `:1200`。`check-citations.mjs` は
  どちらも見ない。

**Minor 4 件:** 639/361 の層別をどのテストも固定していない／番人の場の許可一覧に完全性の検査が無い／
entry シャードの `eq` の除外が位置で決まる id に依る／`heavy:power` の設定は測っただけで緑を見ていない。

### §11.2 修正の波

`f1f93ea`（1 コミット、7 本——Rust のテスト 1・reference のテスト 2・生成器と参照 2・文書 2）。

- **I-1:** 引用 4 箇所（`corpus_opcorr.py`・`generate_corpus.py`・計画 `:15`・設計書 `:53`）を、**文書 2 本を
  名指し、どちらが何を述べているかを書く**形に直した。
- **I-2:** 計画 `:136`/`:144` を 480 → **348 + 132**（日付つきの註で、設計書 §3.3 とこの台帳の裁定 E を引く）。
  `:48` の `engine_table.rs` の引用を `:1188` → **`:1200`**（現物で確認）。
- **Minor 1:** 層別 `{control: 1000, discriminating: 639, same-level: 361}` を
  `reference/tests/test_generate_corpus.py` で固定した。**赤の確認あり**——640/360 に崩して `AssertionError`
  を見てから戻した。
- **Minor 2:** `SEQUENCE_FIELDS` の**完全性の assert** を `crates/calcarc-core/tests/corpus_refused_presses.rs`
  に足した。**赤の確認あり**——追跡外の使い捨てコーパスに、許可一覧に無い文字列配列の場を置いて FAILED を
  見た（`corpus/generated/` は終始 clean）。これで、将来 列を持つ新しい場が増えたときに**番人が黙って
  飛ばす**ことがなくなる。
- **Minor 3:** 小さい直しだったので直した——entry シャードの `eq` の除外を、位置で決まる id から
  `case["keys"] == ["1","2","neg","add","3","eq"]` に変えた（兄弟のテストと同じ形）。
- **Minor 4 は直していない**（§11.4 の裁定）。

### §11.3 限定再レビュー（sonnet、`4b4b1d2..f1f93ea`）

**All findings addressed。新しい指摘 0。**

- **I-1 の読みを、再レビュー役が自分で両方の範囲を読んで確かめた**——`base-spec.md:333-343` が述べるのは
  `xʸ` の右結合（`:333-334`）と `nPr`/`nCr` の位置（`:343`、`× ÷` より上・`xʸ` より下・左結合）だけで、
  **`× ÷` 対 `+ −` には触れていない**。`numerical-policy.md:673` の `+ − = 1, × ÷ = 2, xʸ = 4` が、
  その半分の唯一の出どころである。`git grep -n "333-343"` の 4 箇所すべてが、いま 2 本の文書を名指し、
  どちらが何を述べているかを正しく割り当てている。
- **Minor 1 の赤は、再レビュー役が自分のやり方で確かめた**——写しではなく実ファイルの定数を `{999,1}` に
  崩し、実際の値 639/361 との差で `AssertionError` を見た（＝この assert は**生成されたファイルを読んで
  いる**。規則の再計算ではない）。戻して緑。
- **Minor 2 の赤も、別の名前の場で確かめた**——`mystery_seq` という許可一覧に無い場を持つ使い捨ての
  コーパスファイルを置き、`left: {"keys","left","mystery_seq","right"}` vs
  `right: {"keys","left","right"}` で失敗させた。使い捨てを消して緑。
- 空振りの assert・テストコードに書かれた許容や即値・実測から離れた文書の数は、いずれも無し。
- `git status --porcelain` は赤の確認の前後とも空。reference の pytest **538 passed**、
  `cargo test -p calcarc-core --test corpus_refused_presses` **1 passed**。

（**PR D の限定再レビューとの違いを記録しておく。** あちらは JSON の写しに対して集計を組み直して「落ちる」と
報告し、実際のテストを走らせていなかった——その報告を私が台帳に写して偽の理由を作った（§10.3 の訂正）。
こちらは**実物のテストを、別の崩し方で**走らせている。「赤を見た」は、**何を走らせて赤を見たか**まで
確かめる必要がある。）

### §11.4 裁定（26 件、出た順。「決めたこと — なぜ — 誤っていたときの代償」）

**計画の裁定（R1〜R8）**

- R1 鎖を公開の優先順位で木に組む（設計書は「木 → 平坦」）— 同じ Num/Bin の木・同じ evaluate、手順は
  エンジンと別 — 誤ればアンカー 4 例で赤。
- R2 訂正前の読みは「最初の誤り」の段と結合で読む — 監査の 3 例と 20 が合う — 誤れば discriminating の
  選び方がずれる（期待値は変わらない）。
- R3 訂正前の読みが OutOfShard なら discriminating — 違う結果であることに変わりない — 誤れば層の数が少し動く。
- R4 `entry-000033` のキー列を `1 2 neg add 3 eq` → `-9` に — `heavy:ui` は disabled のボタンを押せない
  （`Key.tsx:90`）、拒否は `engine_table` が見張る — 誤れば「数字が拒まれる」の表示の検査が重量級から
  消える（E2E と `engine_table` に残る）。
- R5 番人は「全コーパスで `refuses` の押下 0」を `cargo test` に — 製品の定義を import、毎 PR で走る —
  誤れば、将来わざと拒まれる列を置くときに番人を書き換える必要がある。
- R6 新シャードは `levels` なし — `NINE_SHARDS` だけが見る — 誤れば science のカバレッジの検査が拾わない
  （対象外なので正しい）。
- R7 変異は `match None::<ReplaceBase>` — `Some` の枝に届かない＝修正前の差し替え — 誤れば検出力が 0 と出て、
  実測で気づく。
- R8 赤の確認は `eb94e72` の使い捨て木に PR C の `reference`/`corpus`/`heavy` を載せて — Rust は main の
  まま — 誤れば Rust の番人（T3）が main で赤いが、それは赤の確認の対象外。

**格子と生成器の裁定（A〜E）**

- A 設計書 §3.5 の文言どおり、**葉に縛るのは組合せの被演算子だけ**にする（`xʸ` の底 2〜9・指数 2〜3 は
  「葉のときの範囲」であって、被演算子を葉に縛る規則ではない）。簡約書が §3.5 より厳しかった——私の
  書き損じ。これで (pow,pow) は生き、`2 ^ 3 ^ 2` = 512 / 訂正前の読み 64 が使える — 誤れば `^` の複合
  被演算子で範囲外の中間値が出るが `_within_range` が捨てる。
- B **段に演算子が 1 つしか無いとき（pow の `^`）に限り、誤り = 正しい の押し直しを許す**（`3 ^ ^ 2` の形）。
  ほかの段は「誤り ≠ 正しい」を保つ — 誤れば (·, pow, pow) の 24 セルが埋まらない。
- C 除外するセルは**手で並べない**。1 つの構造の規則から生成器が計算し、(i) 到達可能なセルが全部埋まること、
  (ii) 除外の集合が計算どおりであること、の 2 つを assert する。到達可能は 480 − 72 = **408** — 誤れば表に
  出る（assert が食い違う）。**→ この数は裁定 E で 348 に改められた。**
- D Task 4 への持ち越し: `corpus.spec.ts` が precedence-000 以外の全シャードで `needsPrecedence` を 0 に
  固定している。新しいシャードはそれを動かす。
- E 到達可能なセルの集合は **348**。手で並べず、**2 つの規則から生成器が計算する**——(a) 組合せの被演算子は
  葉、(b) 2 通りの読みが木として違いうる。両方向に assert する（到達可能は全部埋まる／死んだ集合は計算
  どおり）。列挙そのものを単体テストにも置き、生成器の集合と突き合わせる — 誤れば、将来ふるいや葉の規則を
  変えたときに黙って穴が開く（assert が鳴る）。
- 設計書 §3.3 の「段の 3 つ組は全列挙（80 組）」と §3.6 の「格子 480 セル」は、実測で 348 に直す
  （Task 7 で日付つきの訂正。理由と測り方も書く）。

**タスクのレビューを受けての裁定**

- Minor（mypy の対象外）は直さない——兄弟の `corpus_*.py` と同じ慣行（型検査の範囲は `pyproject` の
  `*_ref.py`）で、範囲を広げるのはこのタスクの外 — 誤れば Group/Chain の型の誤りを mypy が拾わない
  （テストが拾う）。
- 限定再レビューは controller が差分を直接読んで行う（錨の値は算術で検算: 2×(8−(3+2))=6、(2+3)×4=20）—
  2 ファイル 80 行の修正に別の席を立てる手数が見合わない。
- Minor 1（`test_generate_corpus.py` の docstring が「格子 408 セル」のまま＝裁定 E の前の数）は**直す** ——
  検査は緑のまま理由だけが嘘になる形。数は 348。
- Minor 2（新しい公開の `opcorr_reachable_triples` / `opcorr_cells` に `独立:` の行が無い）は**直さない**——
  規律は「参照実装の公開関数」＝期待値を作る関数に掛かるもので、この 2 つは格子を数えるだけ。同じファイルの
  16 個の生成器にも付いていない — 誤りなら、次に独立性の規律を広げるときに一緒に直す。
- Minor 3（テストの再計算が同じ 2 規則に依る）は受ける——規則の読み違いは両方に同じように効くが、
  **除外したセルに件が出たら鳴る assert** が経験的に捕まえる。
- Task 4 のレビューは、**環境の穴を私が埋めてから**出す（`pnpm wasm` → 型検査 → `--list` → `pnpm heavy`）。
  証拠が欠けたままレビューさせない。
- 3 回目の走行を回す。**後片付けの前に、要約 JSON と全ログを作業木の外へ写す** — 誤りなら §3.8 に書く数が
  1 つ欠けたままになる。
- 監視役の 11 分の枠を数分だけ保留して、変異が入ってから 1 回だけ回す — 変異が無い状態で回すと新しい変異の
  数が取れず、11 分をもう一度使うことになる。
- 既存の `minRate` には手を伸ばさない（`precedence-collapse` の precedence-000 は実測 54.95% でも 0.274 の
  まま）。足すのは新しいシャードの行だけ。floor は実測の約 12% 下に置く（実測ぴったりだと揺らぎで赤くなる）。
- Minor（`review-constraints.md` の T6 の「実測を 1/1000 で切り捨て」が、後から出した「実測の約 12% 下」の
  裁定と食い違う）は**直す**——制約の文書が古い裁定を指したままだと、次のレビュー役が違う物差しで測る。

**Task 7 と締めの裁定**

- 行内の印を足す（前例 `:1940` の形に合わせる）。**数は 1 つも直さない——測った値は測ったまま**で、足すのは
  文脈だけ。**私自身がこの比較で誤報を出しかけた**（272 対 577）ので、警告が届かない場所に置いてあるのは
  欠陥である — 誤れば古い数を裸で読んだ人がまた同じ誤報を出す。
- Minor（`task-7-report` が PR 説明の項目を飛ばした件）は**却下しない・直させもしない**。PR 説明は作業場の
  外に在り、push と PR は利用者の手番なので枝には載らない — 誤れば PR 説明が枝に混じるだけ。
- 先端が `c2704d4` → `4b4b1d2` に動いたが、**スイープは回し直さない**。差分が `docs/corpus-measurements.md`
  ただ 1 本・8 行の追加のみで、コードにも生成物にも触れていないため、`c2704d4` の全緑はそのまま先端の緑で
  ある — 誤れば「緑を見ていない先端」を PR に出すことになるが、差分が markdown 1 本であることは印字で
  確かめてある。
- Minor 4（`heavy:power` の設定が測っただけで緑を見ていない）は**直さない**。`heavy:power` は 13 分で
  PR ごとの CI には載らず、マージ前の Heavy corpus 手動起動が既にこの PR の受け入れ条件として書いてある —
  誤れば設定の誤りが手動起動まで見つからないが、その起動は必須条件なのでマージ前には必ず露出する。
- Minor 1〜3 は直す。特に Minor 2（番人の場の許可一覧に完全性の検査が無い）は、**将来の場を黙って飛ばす**
  形で、このリポジトリで最も繰り返している欠陥 — 誤れば手数が増えるだけだが、放てば番人が黙って検査を止める。

### §11.5 検査の記録

- **全スイープは `c2704d4` で ALL-OK**: cargo の ok 行 25・web 543・heavy 316・`pnpm heavy` 267・
  reference 538・tools 3 段・作業木は空。
- **修正前のエンジン（`eb94e72`）での赤の確認**: 走行の合計 `454 of 2000 cases disagree`。
  `discriminating` 454/639・`same-level` 0/361・`control` 0/1,000。止める条件は発火しなかった。
- **検出力（`heavy:power`、13 分 15 秒）**: `operator-correction-revert` 454（22.7%、floor 0.20）、
  `precedence-collapse` 656（floor 0.29）、`associativity-flip` 726（floor 0.32）、
  `display-digits` 572（floor 0.25）。既存 19 本は 2026-09-11 の実測と一致（金融は 2 シャードの和で比較）。
- **修正の波のあとの先端 `f1f93ea`**: `cargo test --workspace` = **ok 行 25・失敗 0**（`c2704d4` の全スイープ
  と同じ数——番人に足した新しい assert が他を乱していない）。`cargo fmt`・`clippy -D warnings`・
  reference の pytest **538**・`ruff check`／`ruff format`・`mypy`・`check-citations`（宙に浮いた引用 0）も緑。
  `corpus/generated/`・`testdata/`・`crates/*/src`・`uv.lock` は無変更。
- **`f1f93ea` で web と heavy の段は回し直していない。** この波は Rust のテストに触れているので cargo は
  workspace 全体を回したが、`web`・`heavy` の入力は 1 バイトも動いていない（`pnpm heavy` はコーパスを読むが
  `corpus/generated/` は無変更）。

### §11.6 マージの条件（PR の本文にも書いてある）

- **マージの前に Actions で Heavy corpus を手動起動する**（この枝の先端で）。重量級は PR の CI では走らず、
  21 枚目のシャードはその走行でしか検証されない。
- **いまの main は重量級の照合が `entry-000033` の 1 件で赤い**（#138 の F5 で `3` が拒まれるため）。
  この枝がその 1 件を打てる形に直す。手動起動は**この枝を指定して**回す。
