...
**【2026-09-23 追記】この仕事は 0.7.0 で出荷済みである。** 枝
`feature/two-shades-of-off` は PR #109 で `main` に入り、`b09188e`（Call it 0.7.0）で
版が付いた。**下の状態表と「次の一手」は 2026-09-02 の座標のまま残っていた**
——**実装は済んでいて、台帳だけが「未着手」と言い続けていた**（0.9.6 の棚卸しで
見つけた。[`2026-09-23-backlog-inventory.md`](2026-09-23-backlog-inventory.md) §2-(b)）。
**残っているのは §「known gap」の暗テーマの検査 1 件だけである。**
...
| B | 4 見た目（形 ＋ 読み上げ） | **完了**（`b004ef1`） |
| B | 5 撮って判断 | **完了**（破線はやめた。判断は `Key.tsx` の註に在る） |
| C | 6 検査を 3 群へ | **完了**（`921f8d7`。`keypad-shell.spec.ts` の 3 本） |
| C | 7 対比と押下台帳 | **完了**（押下台帳は `docs/corpus-measurements.md`） |
...
## 次の一手（Task 4）——**【2026-09-23】済んだ。この節は当時の指示の記録である**
