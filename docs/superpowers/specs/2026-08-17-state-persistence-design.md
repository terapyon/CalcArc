...
**3 パネルとも WASM の読み込みが終わるまで何も描画しない**
（`ScientificPanel.tsx:60` ほか）。**復元が 1 フレーム遅れて見える瞬間は無い。**
...
を採る。** 理由は「非同期だと初期値が一瞬見える」ではない——§2 のとおり 3 パネルとも
WASM 待ちで描画を止めているので、その瞬間は存在しない。**理由は、数十バイトのために
非同期の待ち行列をもう 1 本増やす利益が無いこと**である。読み書きが同期で済み、
復元経路が分岐しない。
...
### Data Scale / Finance — `useState` の初期値

`useState<Primary>(loaded.dataScale.primary)` のように、読んだ値をそのまま初期値に
する。パネルは WASM 待ちで描画を止めているので、**初回描画から復元後の値**になる。
