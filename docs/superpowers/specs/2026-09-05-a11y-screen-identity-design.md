...
日付: 2026-09-05
状態: **裁定済み（2026-09-05）。§12 の #1 と #3 は利用者が承認、#2 は監視役が
既定を選び、#4 は実装後に実機で聞く。** 実装は `docs/superpowers/plans/2026-09-05-a11y-screen-identity.md`。
...
**利用者から出た別案**: 「`<h1>` を隠さず、小さく `CalcArc v0.8.0` と出して、
フッタを `@terapyon GitHub` にしてリンクを残す」。**現物に当てて、採らないと決まった。**

| 当てたこと | 現物 |
|---|---|
| フッタのリンクは既に在る | `web/src/ui/Footer/Footer.tsx:13-22` の `<a href="https://github.com/terapyon/CalcArc">CalcArc {__APP_VERSION__} @terapyon</a>`。**「リンクを残す」は既に成立している** |
| 見える 1 行の代償 | `web/src/ui/Footer/Footer.module.css:32-33` に**別の日に別の目的で測った同じ数**が在る——「390×844 の Finance の余白は **16.31px**」。**今日の実測 16.3125 と一致する**。**見える 1 行はだいたい 16px を使うので、Finance の余白はそれで消える** |
| 見出しとしての中身 | **`CalcArc v0.8.0` は全画面で同じ文字列である。** 見出しへ飛んでも `関数電卓` と `金融計算` が区別できない——**見出しは在るが、何も言っていない**。§4 が果たそうとしている目的そのものを果たさない |
| 版数の置き場 | フッタと見出しの**2 か所**になる。版数は 6 か所を揃える対象であり（CLAUDE.md）、7 つ目を作ることになる |

**利用者はこれを聞いたうえで §4 の案（隠したまま・中身は画面名）を選んだ。**
**「隠す」は手抜きではなく、目的（現在地が分かること）と縦の予算の両方から出た
決めである。**
...
| 干渉するか | 結論 | **実装後の実測**（2026-09-05、`7afa982`。下の再測） |
|---|---|---|
| `<h1>` が 2 つになる | **ならない。** `History` が持つのは `<h2>` 1 つで、`<h1>` は `App` が持つ | ならなかった。見出しは `["H1:関数電卓", "H2:履歴"]` の 2 つ |
| 見出しの階層が飛ぶ | **いまは飛んでいる**（`<h2>` の上に `<h1>` が無い）。**§4 がこれを直す** | 埋まった。`<h1>` → `<h2>` の順である |
| `main.firstElementChild` を見る検査 | **★ 壊れる。§9-1 を読むこと** | 直した（`:scope > :not(h1)`）。Finance の余白は **16.3125px** で、常設化の前後どちらも同じ |
| 履歴の面で `<h1>` が消える | **消えない。** `App` が持つので、パネルの中身に関わらず在る | 消えなかった |
| 履歴の面で live 領域が無くなる | **§5 の領域だけは在る。** いまは 0 件なので、**増える方向である** | **★ 外れた。2 つ在る。** §6 が途中で範囲に入ったので、`UpdateToast` の「更新のお知らせ」も常設になった。**設計書のこの行は 1 つと書いていた** |

### §8.0 **実装後の再測**（2026-09-05、`7afa982`）

**同じ手順で測り直した**（`#scientific` → 第2面に切り替え → 履歴）:

```
見出し          ["H1:関数電卓", "H2:履歴"]
ハッシュ        #scientific（変わらない）
document.title  関数電卓 | CalcArc（変わらない。§8.1 の裁定どおり）
main の子の数    2（<h1> と パネル）
live 領域        2 件  DIV[polite]:画面の切り替え   ← App が <main> の外に持つ（§5）
                      DIV[polite]:更新のお知らせ   ← UpdateToast（§6）
                <main> の中は 0 件
```

**履歴の面に入る前は 7 件**（`DIV[polite]:画面の切り替え` `SPAN[polite]:角度の単位`
`SPAN[polite]:数の表記` `SPAN[off]:計算の途中経過` `SPAN[off]:表示形式`
`OUTPUT[polite]:display-main` `DIV[polite]:更新のお知らせ`）。
**`Display` ごと置き換わるのでパネルの 5 つは消え、パネルの外の 2 つだけが残る。**

**切替の通知は履歴の面では鳴らない。** 履歴は route ではないので `route` が
動かず、領域は空のまま（実測: `""`）である。**これは §8.1 と同じ穴の別の顔**
であり、**§8.1 の裁定（利用者が承認）にそのまま乗る**。

**番人**: `screen-identity.spec.ts` の
`the history screen keeps the two live regions that live outside the panel`
（§10 の #18）。**変異で赤を実測した**——通知領域を `<main>` の中へ移すと
`inMain: true` で赤、`UpdateToast` に `if (!waiting) return null` を戻すと
1 件になって赤。
...
### §9-4. **★ この §9 の一覧が足りていなかった**（2026-09-05、実装で判明）

**§9 は「3 本」と数えていたが、実際は 9 本だった。** `UpdateToast` の常設化で
**`web/tests/e2e/update-toast.spec.ts` の 8 本のうち 6 本が赤くなった**
（実装で回して分かった）。原因は 2 つとも常設化そのものである:

- **空の領域は高さ 0 なので Playwright は hidden と見る** → `toBeVisible()` の 3 本
- **領域はいつでも 1 件なので `toHaveCount(0)` が成立しない** → 3 本が
  「いまと逆のこと」を言うようになる

**採った直し**: assertion は 1 つも変えず、**locator の 1 行だけ**を
「領域」から「その中の箱」に変えた
（`getByRole("status", { name: "更新のお知らせ" }).locator(":scope > div")`）。
**役割と名前はいまも見ている**——箱はその親の子としてしか取れない。
別案（`aria-label` と class を `waiting` で出し入れして E2E をまったく触らない）は、
**領域が名前を失う時間ができる**うえ `App` の常設領域と流儀が食い違うので採らなかった。

**そのうえで、1 本が空主張になっていた。**
`without the preview parameter there is no toast` は「箱が 0 件」しか言わず、
**領域ごと消えても緑になる**——常設化する前の姿そのものである。
`toHaveCount(1)` と `toBeEmpty()` を足し、**`return null` を戻す変異で赤を実測した**
（足す前は緑のままだった）。手本は `eng-notation.spec.ts:34,58,88`。

**なぜ数え違えたか。** §9 は**ファイルを読んで**影響を数えた。
**回して数えたわけではなかった。** 「計画の一覧は grep から起こす」の同型で、
**読んで作った一覧は漏れる**。次に同じ形の変更を設計するときは、
**先に既存の検査を実際に落としてから数える。**
...
| 18 | **履歴の面でも live 領域が残る**（パネルの外の 2 つ） | **E2E** | `screen-identity.spec.ts`: `the history screen keeps the two live regions that live outside the panel`（件数と綴りと `<main>` の外であることを同時に見る） | 通知領域を `<main>` の中へ移す／`UpdateToast` に `if (!waiting) return null` を戻す（**2 つとも変異で赤を実測した**。§8.0） |
...
| # | 論点 | **決着（2026-09-05）** |
|---|---|---|
| 1 | **13 個の画面名の綴り**（§2 の表）。**利用者に見える文字列である**。特に**新しい 2 つ**（`関数電卓` / `金融計算`）と、**新しい語形の 2 つ**（`データ量の換算` / `データ量の規模`） | **★ 利用者が承認。§2 の表のまま。** **実装で綴りを変えない**——承認されたのは組み立て規則ではなく**綴りそのもの**である |
| 2 | 通知の文面「**〜に切り替えました**」 | **監視役が既定を選んだ**（利用者の裁定ではない）。**利用者の別案が出たら差し替える**——綴りは 1 か所に在るので後から動かせる |
| 3 | **履歴の面がタイトルを変えないこと**（§8.1）。変えるなら履歴をルートにする話になり、URL 設計の変更である | **★ 利用者が承認。理由ごと受け入れられた。** §8.1 をそのまま実装する |
| 4 | **切替の直後、`polite` の読み上げが長くなること**（§5）。既存の `polite` 領域（角度・記法・表示）と順番に読まれる。**CI では確かめられない** | **★ 未決のまま残す。** 実装を止める理由にはしないが、**実装が終わっても未決である**——出してから実機で聞いて決める。**この行を消さないこと** |
| 5 | **通知の文面が消えずに残ること**（§5。**2026-09-05 のレビューの検分**）。`App` は一度入れた `${画面名}に切り替えました` を消さないので、**読み上げで頁を線形に辿る利用者は、切替のずっと後にもその一文を頁の中身として踏む** | **★ 未決のまま残す。#4 と同じ束（実機で聞いて決める）。** **いま直さない**——単純に消すと「同じ画面名に 2 度切り替えたとき鳴らない」等の別の癖が生まれるので、単純な直しではない。**実機確認の項目として残す** |
...
- ~~**`§4.1` の縦の予算は 1 route（Finance、390×844）でしか測っていない。**~~
  **測った（2026-09-05、`7afa982`）。** `viewport-budget.spec.ts` の
  **11 route × 2 幅 = 22 本**が緑である（`fits in one screen at 390x844` 11 本 +
  `does not spill sideways at 360px` 11 本。E2E 全体は 237 本が緑）。
  **11 であって 13 ではない**——`#scale/llm` と `#convert/currency` は
  同ファイルの冒頭に理由つきで巡回の外に置かれている。
  **同じ 22 通りで直接も測った**: 隠した `<h1>` の高さは **全 22 通りで 1px**、
  縦のはみ出しは 0、横のはみ出しも 0。Finance の余白は **390×844 で 16.3125px**
  ——**`<h1>` を置く前と同じ数**である（§4.1a が別の日に別の目的で記録した
  16.31px とも一致する）。
- ~~**`§6` の「空の領域は高さ 0」は見立てである。**~~
  **測った（同上）。** 空のときの `<div role="status" aria-label="更新のお知らせ">` の
  高さは **全 22 通りで 0px**（幅は親いっぱいの 390 / 360、子要素は 0 個）。
  `.toast` が `position: fixed` で通常フローから抜けているという推測は当たっていた
  ——**推測が当たっていたことと、測ったことは別である。**
