...
**実装中の訂正（2026-08-20、feature/heavy-scientific-ui-report）**: 実測が 2 か所で
本 spec の記述を覆した。該当節に**【訂正 N】**の印を付け、旧記述と変えた理由を
その場に残す（§7.2 Finance の費用が高い理由、§8.1 エラー経路の枠の数）。
**旧記述は消していない**——理由だけが静かに腐るのを避けるため、何がどう偽だったかを
その場で読めるようにしてある。
...
**【訂正 1（2026-08-20、Task 6 実装後）】結論は当たっていたが、理由が偽である。**
旧記述は「Finance は打鍵ではなく欄への入力なので」を費用が違う理由としていた。
**Finance に欄への入力は無い**——`FinancePanel.tsx` は `Keypad` を描画して
`pushDigit` / `pushOperator` を呼び、既存の e2e もボタンを押して駆動している
（`fill()` を使う箇所は 1 つも無い）。1 件あたりの費用が違うことは実測で当たって
いた（**0.97 秒/件 対 科学計算の 0.53 秒/件**）が、高い理由は 2 つとも別である:

1. **盤面がモードをまたいで値を持ち回り、`AC` は打っている項目しか消さない。**
   前のケースの残りが次のケースに混ざるので、**1 件ごとにページから開き直す。**
2. **金額が 20 桁に及ぶ。** 16 件で **417 クリック**（1 件 26.1 回）。

増分そのものは見込みどおり 1 分未満だった（**19 テスト 700.3 秒 → 36 テスト
713.4 秒、増分 13.1 秒**）。実測は
[docs/corpus-measurements.md](../../corpus-measurements.md) の
「Finance を実画面から通した」節にある。
...
**【訂正 2（2026-08-20、Task 7 実測 → 監視役の裁定）】「5 つに分ける」は偽で、
正しくは「6 経路 + 番兵」である。** 旧記述の 5 枠は上の表のとおりで、この節を
そのまま実装すると**コーパスの実物が 1 件はみ出す**。実測: コミット済みコーパスの
エラー期待値は **394 件**で、内訳は `errors-000.json` 28 件 / `finance-000.json`
361 件（`SyntaxError` 270 + `Overflow` 91）/ `data-scale-000.json` 4 件 /
`entry-000.json` 1 件。最後の 1 件（`entry-000035`、キー列 `3 . .`、`SyntaxError`）が
上の 5 枠のどれにも入らない。

足す枠は **「打鍵の途中の構文エラー」**——`=` を押す前に、打っている数そのものが
構文として壊れる経路である。他の枠は打ち終えた式を計算してから撥ねるが、これは
計算に届かない。**件数 1 でも名前のある枠にする**: 名前のある 1 は番兵の中の 1 より
読者に多くを語る（番兵は「どこにも入らなかった」としか言えない）。

| 枠 | 出どころ |
|---|---|
| Scientific の定義域エラー | `errors-000.json`（シャード名で選ぶ。領域は `display`） |
| Finance の `SyntaxError` | call シャードの集計 |
| Finance の `Overflow` | 同上 |
| Data Scale の入力エラー | `data-scale-000.json` |
| **打鍵の途中の構文エラー** | `entry-000.json`（同じくシャード名で選ぶ） |
| **走行そのものの失敗** | A の `heavy-run.json` |

**番兵は 6 枠化後も「期待 0」で恒久に残す。** 枠の集合が完全であることは原理的に
証明できない——いまのコーパスで空だというだけである。新しい経路が生えた日に、その
件数が既存の枠へ加算されるのではなく**名前ごと本文に浮く**仕組みが要る。ピンの
テストは「**枠 6 つ + 番兵 = 総数**」と「**番兵 = 0**」を両方主張する
（`report.spec.ts` の `every error expectation in the committed corpus lands in
one of the six frames`）。「期待 0」の見張りは何も主張しなくても緑になるので、
**番兵が実際に 1 になって本文に行が現れることを赤確認で 1 度実測した**
（`entry-000.json` を枠から外す変異。実出力は実装報告に貼ってある）。
