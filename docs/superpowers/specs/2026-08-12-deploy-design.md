...
2. スモークが 2 前提（_headers 尊重・最新配信）を実 URL で機械検証している。
   **赤の対**: 刻印照合は「わざと違う SHA を期待させる」一時変更で赤になる
   ことを初回実行時に確認する（スモーク自体が空虚でないことの実証。
   ヘッダ検査は _headers を触らずに赤を作れないため、ロジックの素振りを
   ローカル preview で行う——preview は全レスポンスに一律 no-cache を返す
   ため、no-cache 系 2 脚は偶然緑になり、赤は immutable 脚の不一致から
   出た（Task 2 の素振りで実測）。3 脚は同一の形なので、どの脚の赤でも
   curl→grep→exit の生存確認としては等価——結論は変わらない）。
