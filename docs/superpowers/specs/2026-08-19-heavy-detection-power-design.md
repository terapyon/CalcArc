...
**追記（2026-08-19）。** 実装は当初この検査を `heavy-run.json` の `expected`
との突き合わせで書いており、しかも欠け方向しか見ていなかった。`expected` は
走行が `corpus/generated/` を読んでその場で導いたものなので、**シャードの
ファイルが 1 枚消えると `expected` も一緒に縮む**——この節が塞ごうとした穴が
そのまま開いていた。相談役の指摘で気づき、`ALL_SHARDS`（15 枚）を定数として
置き、両方向の完全一致に直した。枚数だけの assert は採らない（1 枚消えて
1 枚増えた走行が通り、何が消えたかを言えない）。
...
5. `expectShards` が空（`nothing` 期待）:
   ...
   - **シャードが実際に反応した** → 赤。理由は「レポートの主張が嘘だった」
     （`claim-was-false`）。1〜4 の赤とは意味が違うので、文言を分ける。
   - **どのシャードも反応していないのに走行が非ゼロで終わった** → 赤。
     理由は `measurement-failed`。

**訂正（2026-08-19）。** この節は当初、5 の「それ以外」を**すべて**
「レポートの主張が嘘だった」に振っていた。それは誤りである。どのシャードも
反応していないのに走行が非ゼロで終わるのは、シャード比較**以外**の失敗
（タイムアウト、別のテストの失敗）で走行そのものが壊れただけで、
レポートの「踏んでいない」という主張は破られていない。この節自身が
「1〜4 は測れていない、5 以降は測った結果が期待と違う」と定めている以上、
壊れた走行を検出の結果として報告するのは枠組みに反する。

**実例がある。** 2026-08-19 の 2 回目の `pnpm heavy:power` で
`associativity-flip` がこの枝に落ちた。原因は変異ではなく、1 回目が書いた
`detection-power.json` が残っていてレポートの検査を落としたことだった。
`claim-was-false` と報告していたら、「結合方向は踏んでいない」という
レポートの主張が嘘だと結論していた——**実際には嘘ではなかった。**

同じ非対称が 6 の側（`expectShards` が非空で反応 0 かつ非ゼロ終了）にもある。
そこも `caught-nothing` ではなく `measurement-failed` とする。隣り合う 2 つの枝に
違う意味論を割り当てない。
...
**追記（2026-08-19、Task 7 の実走で確かめた）。** 反応しない理由は許容誤差である。
値シャードは 9 枚あり、`cancellation-000.json` だけが `tolerance.rel = 1e-6`、
残る 8 枚は `5e-10` である（`corpus/generated/*.json`）。値ケースの比較は
`classifyComplex`（`web/tests/heavy/corpus.ts`）が**相対誤差だけ**で行い、
`abs` を見るのは期待値が厳密に 0 のときに限られる——そしてこのシャードに期待値 0 の
ケースは 1 件も無い（実測 0 / 2000）。`DISPLAY_DIGITS` を 10 から 9 に落として生じる
相対誤差は 9 桁丸めの半 ulp、すなわち **5e-9 以下**である。

**余裕も測った。** 「変異が足す誤差が小さい」だけでは足りない——ベースラインの
相対誤差が 1e-6 のすぐ下に居るケースが 1 件でもあれば、原理上は跨げてしまう。
このシャードの実測最大相対誤差は **7.67e-7**（`heavy-report.md` 付録、2026-08-19 の
走行）で、閾値までの余裕は 2.33e-7。変異が足せるのは 10 桁側と 9 桁側の半 ulp の和
＝ 5.5e-9 が上限だから、最悪でも 7.73e-7 で 1e-6 に届かない。**余裕は摂動の
約 42 倍ある。** よってこのシャードは閾値を跨ぎようがない。**たまたま反応しなかったのではなく、この変異では
原理的に反応しない。** 逆に 5e-9 > 5e-10 だから、他の 8 枚は跨ぐ。

したがって `cancellation-000.json (values)` を `expectShards` に足せる条件は
「コーパスが変わったら」ではなく「このシャードの `rel` が 5e-9 より厳しくなったら」
である。B+C で層別化しても `rel` を動かさない限り、この期待は動かない。

`corrections-000.json` と `equivalence-000.json` も `expectShards` に無いが、
これは別の理由による。この 2 枚は全 2000 件が `kind: "equivalence"` で、
値ケースを 1 件も持たない。`expectedSummaryNames()`（`report.ts`）は
`values.length > 0` のときだけ `(values)` の名前を作るので、この 2 枚に
`(values)` の要約はそもそも存在しない——総件数 0 で並ぶのではない。
`data-scale-000.json` と `finance-000.json` は全件が `kind: "call"` である。
