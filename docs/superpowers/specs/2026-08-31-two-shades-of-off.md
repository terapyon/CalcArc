...
**★ この裁定の帰結**: **見た目は 3 つになる**（生きている／永久に押せない／
いまだけ押せない）が、**「押せない側」は 2 群**である——`—` は永久側に合流する。
...
**★ 裁定（2026-08-31）: (b) ＋ (d)、(a) は補助。** **見える手がかりと、読み上げの
手がかりを両方**置く。**濃さだけにはしない。**

**★ 条件つきである**——**67px のキーで破線が見えるかは撮ってから判断し、
見えなければ (a) ＋ (d) に落とす**。**「見えなかった」も正当な結末**で、
そのときは**濃さ ＋ `aria-describedby`** になる。
...
## §4 裁定（**2026-08-31、ユーザー。両方とも決着済み**）

| | 決まったこと |
|---|---|
| **予約スロット `—` の扱い** | **「永久に使えない」と同じ段**（§1.1 の A）。理由も採用 |
| **何で分けるか** | **形 ＋ 読み上げ、濃さは補助**（§1.2 の (b)+(d)、(a) は併用）。**67px で破線が見えなければ濃さ ＋ 読み上げに落とす**、という条件つき |

**★ plan への申し送り**: **破線を採るなら、検査の `looks` 文字列に `border-style`
（必要なら `border-color`）を足すこと。** **いまの前例は `opacity` と `cursor` しか
読まない**ので、**濃さを変えずに形だけで分けると、3 群が computed style で
分かれない**——**「見た目で分けた」と書いてある検査が、見た目の差を見ていない**
ことになる。
