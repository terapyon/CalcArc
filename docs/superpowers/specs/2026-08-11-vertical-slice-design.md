...
D14 以降の決定は各機能の spec に記録する（最初の例:
2026-08-11-data-scale-design.md §1 = #7 の決定）。
...
**【追記 2026-08-16、S-1】この副作用は意図的に取り消した。** `4` `+/−` `√` は `DomainError` になる（S-1 設計書 §1 の裁定 1）。**内部表現の統一（この節の主題）はそのままである**——`Value` は今も複素数 1 種類で、四則も極座標変換も複素数を扱う。変えたのは**関数の値域**だけで、「関数は実数に閉じる。複素数は入力と四則と表示の機能である」という線引きになった。この段落は、その線引きが引かれる前の判断の記録として残す。
