...
- **期待値はエンジンの移植にしない**（設計書 §0-1）。読むのは `docs/base-spec.md`:333-343 の公開の約束だけである。
...
- **R4** `entry-000033` はキー列を変える: `1 2 neg 3` → **`1 2 neg add 3 eq`、表示 `-9`**。
  ...
  - 拒否そのものは `engine_table.rs`:1188 `refused_after(&["1","2","neg"], "3")` と 3d の E2E が見張る。
