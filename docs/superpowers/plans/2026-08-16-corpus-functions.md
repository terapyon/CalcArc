...
- **テスト断片の import 行は、既存ファイルの冒頭の import に統合すること。**
  以降の各タスクの「失敗するテストを書く」に載っている `import` / `from ... import`
  は、**読みやすさのために断片を自己完結させたもの**である。そのまま 2 つ目の
  import ブロックとして貼ると、既存の import と重複して `ruff check` が
  **F811（redefinition）**で落ちる。**足りない名前だけを既存の import に足す。**
  （Task 1 の実装者が実際に踏んだ。計画の欠陥であって実装者の落ち度ではない。）
