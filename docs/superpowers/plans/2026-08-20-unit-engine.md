...
CONVERT_INPUTS: list[tuple[str, str, str, str]] = [
    ...
    ("10000", "length", "km", "mm"),
    # **half-even の tie 2 件。** 11 桁目がちょうど 5 で、後ろに何も無いこと。
    # 10 桁目が**偶数なら据え置き**（`1.23456789`）、**奇数なら切り上げ**（`1.234567892`）。
    # **半分だけ置くと、half-up への変異で片方しか動かない**——両側を挟む。
    #
    # **【検算せよ】`1.00000000005` は tie ではない。** 11 桁目は 0 で 12 桁目が 5 なので、
    # 余りは 10 桁目の単位の半分に届かず、half の枝を一度も通らない。
    # この値を置くと **spec §6 が警告した「赤確認 4 が空振りする」がそのまま起きる**。
    ("1.2345678905", "length", "m", "m"),
    ("1.2345678915", "length", "m", "m"),
...
**【追加 2026-08-20】小数点キー `dot` を置くこと。** `dataScale.ts` の `PAD` を骨格にすると
**`.` が 1 つも無い**——Data Scale と LLM の項目が整数だからである。**換算の値は実数で、
`1 in = 25.4 mm` も `0 °C = 273.15 K` も打てなくなる**（spec §3.2 の定義値そのもの、§6 の名指しケース）。
位置は `scientific.ts:217-221` と同じ 5 行 3 列目（`dataScale.ts` が「選択」を置いていた予約スロット）。
`units/entry.ts` は `pushDot` を既に持っているので下流の追加は要らない。

**これは裁定 3（`±` キー）とまったく同じ型の穴である**——**spec が名指しした端到端のケースが、
盤面から打てない**。[[end-to-end-cases-need-the-whole-path]]。
**名指しケースを実装するときは、その値を打つのに要るキーを 1 文字ずつ数えること。**

**「5 列 × 5 行」は枠の話であって、セル数ではない。** 実物の `dataScale.ts` の `TYPES` は
**15 セル（3 行）**で、5 行を保っているのは CSS（`grid-template-rows: repeat(5, 1fr)` と
`aspect-ratio: 1/1`）である。**行数は内容ぶんだけにする**——字面どおり 25 セルに揃えると、
温度の単位面（3 単位）で**押せない「空き」ボタンが 20 個読み上げに並ぶ**。
