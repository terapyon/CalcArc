...
**コーパスは、生成器の出力が変わった Task ごとに再生成する。**

最初はこう書いていなかった——「再生成は Task 9 で 1 回だけ。層を足すたびに
再生成すると golden の差分が読めなくなる」。**これは間違いだった。**
再現性ゲート（`test_corpus_reproducibility.py`）は「コミット済みの golden が
いまの生成器から出るか」を見るので、生成器の出力が変わった時点で赤くなる。
Task 1 から Task 9 まで赤を抱えて進むと、**ゲートはその 8 つの Task のあいだ
ずっと何も主張しない。**「テストは何も主張しないことがある」の形そのものである。

差分が読めなくなるという心配も、逆だった。生成器の変更とそれが data に与えた
影響が同じコミットに入るほうが読める。**変更と結果を離すのは、読みやすさでは
なく確かめにくさである。**

Task 1 の実装者が「再現性ゲートだけが赤いが、これは Task 9 まで想定内」と
正直に申告してきたので、想定内にしないことに決めた。以後、各 Task は
`uv run python scripts/generate_corpus.py` を回し、`corpus/generated/` の
差分を**その Task のコミットに含める**。
...
**期間の期待は op に依存する。** `0` は loan・複利のどちらでもエラーだが、
**`1201` は複利だけがエラーで、loan では正常に計算される**——loan の期間に
上限のガードは無い（`MAX_TERM_MONTHS` は逆算の探索打ち切りであって入力の
契約ではない）。設計書 §4.2 の訂正を読んでから水準表を書くこと。
**`loan_*` の 1201 を `SyntaxError` と宣言すると落ちる。落ちたときに期待を
実測へ合わせない**——実物がそうなっているのが正しい。

- [ ] **Step 2: 設計書 4.5 の 17 経路に各 5 件以上の層を作る**
...
（各 Task で再生成しているので、ここで走るのは**件数が 2,000 から 3,500 に
変わったこと**による差分である。）
