...
- **既存レイヤーに影響を出さない。** 既存の `pnpm exec playwright test`（設定なし）が heavy を 1 件も拾わないこと。

  **訂正（2026-08-16、実行後）。** この行は当初「既存 4 レイヤー」と書いていたが、
  **`ci.yml` のジョブは 5 つある**——`rust` / `wasm`（WASM boundary）/ `web` / `e2e` /
  `reference`。実行中の非干渉確認は vitest / 既存 Playwright / pytest / cargo の 4 つで、
  **WASM 境界層（`wasm-pack test --headless --chrome`）を一度も回していなかった。**
  手元では回せない（wasm-pack が取る ChromeDriver 152 と手元の Chrome 135 が不一致。
  CLAUDE.md 記載の既知の罠で、CI はランナー同梱の対で回避している）ので、代わりに
  **影響し得ないことを構造で示した**——`git diff --stat ec13beb HEAD -- crates` が空で、
  ブランチ全体で `crates/` に一度も触れていない。
  **「回した」と言えないときは「触っていない」を示す。**
  次の段階では、検証コマンドの一覧を `ci.yml` から機械的に起こすこと。手で書いた一覧は漏れる。
- コミットメッセージ末尾に `Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>`。**`git push` と PR 作成は行わない。**

### 設計書の「移行」節の訂正（この計画で正す）

**節番号について（2026-08-16、実行後の訂正）。** この見出しは当初「§6」と書いていたが、
訂正の対象は現行の **§5「移行」**である（§6 は「リスク」）。番号が 1 つずれていた。

設計書の「移行」節は「変わるのは `tolerance` フィールドの値で、生成器を直して再生成すれば済む」と書いているが、**これは不正確である。`{abs: 5e-10, rel: 5e-10}` という値は変わらない。** 変わるのは判定側の**解釈**だけで、`abs` が「OR の片側」から「期待値が 0 のときの専用経路」に格下げされる。したがって:
