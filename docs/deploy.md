...
Release に 3 つの証拠と、マニュアルの PDF が添付される。**どれもその走行が
作った物**で、別の走行から持ってこない。

| 添付物 | 何を示すか |
|---|---|
| `release-evidence.md` | タグ・コミット・**各ジョブの結論**（`gh api` が走行から読む。人が書き写さない）。落ちた検査が 1 つでもあれば、この文書は**作られずにジョブが落ちる**——**例外は `Manuals` の 1 つだけ**で、落ちていれば「マニュアル（PDF）は作れなかった」と書いて続ける |
